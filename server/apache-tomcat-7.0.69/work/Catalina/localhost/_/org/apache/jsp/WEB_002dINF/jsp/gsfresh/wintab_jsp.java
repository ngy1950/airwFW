/*
 * Generated by the Jasper component of Apache Tomcat
 * Version: Apache Tomcat/7.0.69
 * Generated at: 2020-03-22 22:46:10 UTC
 * Note: The last modified time of this file was set to
 *       the last modified time of the source file after
 *       generation to assist with modification tracking.
 */
package org.apache.jsp.WEB_002dINF.jsp.gsfresh;

import javax.servlet.*;
import javax.servlet.http.*;
import javax.servlet.jsp.*;
import com.common.bean.CommonConfig;

public final class wintab_jsp extends org.apache.jasper.runtime.HttpJspBase
    implements org.apache.jasper.runtime.JspSourceDependent {

  private static final javax.servlet.jsp.JspFactory _jspxFactory =
          javax.servlet.jsp.JspFactory.getDefaultFactory();

  private static java.util.Map<java.lang.String,java.lang.Long> _jspx_dependants;

  private volatile javax.el.ExpressionFactory _el_expressionfactory;
  private volatile org.apache.tomcat.InstanceManager _jsp_instancemanager;

  public java.util.Map<java.lang.String,java.lang.Long> getDependants() {
    return _jspx_dependants;
  }

  public javax.el.ExpressionFactory _jsp_getExpressionFactory() {
    if (_el_expressionfactory == null) {
      synchronized (this) {
        if (_el_expressionfactory == null) {
          _el_expressionfactory = _jspxFactory.getJspApplicationContext(getServletConfig().getServletContext()).getExpressionFactory();
        }
      }
    }
    return _el_expressionfactory;
  }

  public org.apache.tomcat.InstanceManager _jsp_getInstanceManager() {
    if (_jsp_instancemanager == null) {
      synchronized (this) {
        if (_jsp_instancemanager == null) {
          _jsp_instancemanager = org.apache.jasper.runtime.InstanceManagerFactory.getInstanceManager(getServletConfig());
        }
      }
    }
    return _jsp_instancemanager;
  }

  public void _jspInit() {
  }

  public void _jspDestroy() {
  }

  public void _jspService(final javax.servlet.http.HttpServletRequest request, final javax.servlet.http.HttpServletResponse response)
        throws java.io.IOException, javax.servlet.ServletException {

    final javax.servlet.jsp.PageContext pageContext;
    javax.servlet.http.HttpSession session = null;
    final javax.servlet.ServletContext application;
    final javax.servlet.ServletConfig config;
    javax.servlet.jsp.JspWriter out = null;
    final java.lang.Object page = this;
    javax.servlet.jsp.JspWriter _jspx_out = null;
    javax.servlet.jsp.PageContext _jspx_page_context = null;


    try {
      response.setContentType("text/html; charset=UTF-8");
      pageContext = _jspxFactory.getPageContext(this, request, response,
      			null, true, 8192, true);
      _jspx_page_context = pageContext;
      application = pageContext.getServletContext();
      config = pageContext.getServletConfig();
      session = pageContext.getSession();
      out = pageContext.getOut();
      _jspx_out = out;

      out.write("\r\n");
      out.write("\r\n");

	String theme =(String)request.getSession().getAttribute(CommonConfig.SES_USER_THEME_KEY);

      out.write("\r\n");
      out.write("<!doctype html>\r\n");
      out.write("<html lang=\"ko\">\r\n");
      out.write("<head>\r\n");
      out.write("<meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\">\r\n");
      out.write("<meta charset=\"utf-8\">\r\n");
      out.write("<title></title>\r\n");
      out.write("<meta name=\"viewport\" content=\"width=1150\">\r\n");
      out.write("<link rel=\"stylesheet\" type=\"text/css\" href=\"/common");
      out.print(theme);
      out.write("/css/common.css\">\r\n");
      out.write("<link rel=\"stylesheet\" type=\"text/css\" href=\"/common");
      out.print(theme);
      out.write("/css/wintab.css\">\r\n");
      out.write("<link rel=\"stylesheet\" type=\"text/css\" href=\"/common");
      out.print(theme);
      out.write("/css/theme.css\">\r\n");
      out.write("<script type=\"text/javascript\" src=\"/common/js/jquery.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\" src=\"/common/js/dataMap.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\" src=\"/common/js/configData.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\" src=\"/common/js/commonUtil.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\" src=\"/common/js/ui.js\"></script>\r\n");
      out.write("<script type=\"text/javascript\">\r\n");
      out.write("\tvar $multiList;\r\n");
      out.write("\tvar $tabObj;\r\n");
      out.write("\tvar $pathTitle;\r\n");
      out.write("\tvar tabMaxCount;\r\n");
      out.write("\t$(document).ready(function(){\r\n");
      out.write("\t\t$multiList = jQuery(\"#multiList\");\r\n");
      out.write("\t\t$tabObj = $multiList.find(\"li\").clone();\r\n");
      out.write("\t\t$multiList.find(\"li\").remove();\r\n");
      out.write("\t\t$pathTitle = $(\"#pathTitle\");\r\n");
      out.write("\t\t\r\n");
      out.write("\t\tuiList.UICheck();\r\n");
      out.write("\t\t\r\n");
      out.write("\t\t$winTabLeft = $(\"#winTabLeft\");\r\n");
      out.write("\t\t$winTabRight = $(\"#winTabRight\");\r\n");
      out.write("\t});\r\n");
      out.write("\t$(window).resize(function(event){\r\n");
      out.write("\t\tcheckTabListSize();\r\n");
      out.write("\t});\r\n");
      out.write("\tfunction sizeToggle(self){\r\n");
      out.write("\t\tvar self = $(self);\r\n");
      out.write("\t\tvar src = self.find(\"img\").attr(\"src\");\r\n");
      out.write("\t\tvar filename = src.substring(src.lastIndexOf(\"/\")+1,src.lastIndexOf(\".\"));\r\n");
      out.write("\t\tfilename!=\"ico_lnb_closer\"?self.find(\"img\").attr(\"src\",\"/common");
      out.print(theme);
      out.write("/images/ico_lnb_closer.png\"):self.find(\"img\").attr(\"src\",\"/common");
      out.print(theme);
      out.write("/images/ico_lnb_closel.png\");\r\n");
      out.write("\t\tconsole.log(filename)\r\n");
      out.write("\t\twindow.top.sizeToggle();\r\n");
      out.write("\t}\r\n");
      out.write("\t/*\r\n");
      out.write("\tfunction setPathTitle(pathTitle){\r\n");
      out.write("\t\treturn;\r\n");
      out.write("\t\tif(pathTitle.length > 50){\r\n");
      out.write("\t\t\tpathTitle = \" \";\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\t$pathTitle.text(pathTitle);\r\n");
      out.write("\t}\t\r\n");
      out.write("\t*/\r\n");
      out.write("\tfunction activeTab($tab){\r\n");
      out.write("\t\t$multiList.find(\"li\").removeClass(\"active\");\r\n");
      out.write("\t\t$tab.addClass(\"active\");\r\n");
      out.write("\t\tvar menuId = $tab.attr(\"menuId\");\r\n");
      out.write("\t\twindow.top.changeWindow(menuId);\r\n");
      out.write("\t\t//var pathTitle = $tab.attr(\"pathTitle\");\r\n");
      out.write("\t\t//setPathTitle(pathTitle);\t\t\r\n");
      out.write("\t}\r\n");
      out.write("\tfunction changeWindow(menuId){\r\n");
      out.write("\t\tvar $tab = $multiList.find(\"[menuId=\"+menuId+\"]\");\r\n");
      out.write("\t\tif($tab.hasClass(\"active\")){\r\n");
      out.write("\t\t\t\r\n");
      out.write("\t\t}else{\r\n");
      out.write("\t\t\tactiveTab($tab);\r\n");
      out.write("\t\t\t/*\r\n");
      out.write("\t\t\t$multiList.find(\"li\").removeClass(\"active\");\r\n");
      out.write("\t\t\t$tabs.addClass(\"active\");\r\n");
      out.write("\t\t\tvar pathTitle = $tabs.attr(\"pathTitle\");\r\n");
      out.write("\t\t\twindow.top.changeWindow(menuId);\r\n");
      out.write("\t\t\t$(\"#pathTitle\").text(pathTitle);\r\n");
      out.write("\t\t\t*/\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\tcheckTabListSize();\r\n");
      out.write("\t}\r\n");
      out.write("\tfunction closeWindow(menuId, allType){\r\n");
      out.write("\t\tif(!allType && !window.top.checkWindowModify(menuId)){\r\n");
      out.write("\t\t\treturn;\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\t\r\n");
      out.write("\t\tvar $tabs = $multiList.find(\"[menuId=\"+menuId+\"]\");\r\n");
      out.write("\t\t$tabs.remove();\r\n");
      out.write("\t\tvar tmpActiveMenuId;\r\n");
      out.write("\t\tif($tabs.hasClass(\"active\")){\r\n");
      out.write("\t\t\tvar $tabList = $multiList.find(\"[menuId]\");\r\n");
      out.write("\t\t\tif($tabList.length >= 1){\r\n");
      out.write("\t\t\t\tvar $tmpTab = $tabList.eq($tabList.length-1);\r\n");
      out.write("\t\t\t\tactiveTab($tmpTab);\r\n");
      out.write("\t\t\t\ttmpActiveMenuId = $tmpTab.attr(\"menuId\");\r\n");
      out.write("\t\t\t}\r\n");
      out.write("\t\t}else{\r\n");
      out.write("\t\t\ttmpActiveMenuId = \" \";\r\n");
      out.write("\t\t}\t\t\r\n");
      out.write("\t\twindow.top.closeWindow(menuId, tmpActiveMenuId);\r\n");
      out.write("\t\t\r\n");
      out.write("\t\tcheckTabListSize();\r\n");
      out.write("\t}\r\n");
      out.write("\t\r\n");
      out.write("\tfunction closeAll(){\r\n");
      out.write("\t\tvar tabList = $multiList.find(\"li\");\r\n");
      out.write("\t\tvar menuId;\r\n");
      out.write("\t\tfor(var i=0;i<tabList.length;i++){\r\n");
      out.write("\t\t\tmenuId = tabList.eq(i).attr(\"menuId\");\r\n");
      out.write("\t\t\tcloseWindow(menuId, true);\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\t\r\n");
      out.write("\t\tcheckTabListSize();\r\n");
      out.write("\t}\r\n");
      out.write("\tfunction setPage(pathTitle, title, url, menuId){\r\n");
      out.write("\t\t//setPathTitle(pathTitle);\r\n");
      out.write("\t\t\r\n");
      out.write("\t\t$multiList.find(\"li\").removeClass(\"active\");\r\n");
      out.write("\t\tvar $tmpObj = $tabObj.clone();\r\n");
      out.write("\t\tvar $tmpSpan = $tmpObj.find(\"span\").eq(0);\r\n");
      out.write("\t\t$tmpSpan.text(title);\r\n");
      out.write("\t\t$tmpObj.addClass(\"active\");\r\n");
      out.write("\t\t$tmpObj.attr(\"menuId\",menuId);\r\n");
      out.write("\t\t//$tmpObj.attr(\"pathTitle\",pathTitle);\r\n");
      out.write("\t\t$tmpSpan.click(function(event){\r\n");
      out.write("\t\t\tvar $obj = $(event.target);\r\n");
      out.write("\t\t\tvar menuId = $obj.parents(\"[menuId]\").attr(\"menuId\");\r\n");
      out.write("\t\t\tchangeWindow(menuId);\r\n");
      out.write("\t\t});\r\n");
      out.write("\t\t$tmpObj.find(\"img\").click(function(event){\r\n");
      out.write("\t\t\tvar $obj = $(event.target);\r\n");
      out.write("\t\t\tvar menuId = $obj.parents(\"[menuId]\").attr(\"menuId\");\r\n");
      out.write("\t\t\tcloseWindow(menuId);\r\n");
      out.write("\t\t});\r\n");
      out.write("\t\t$multiList.append($tmpObj);\r\n");
      out.write("\t\t\r\n");
      out.write("\t\tcheckTabListSize();\r\n");
      out.write("\t}\r\n");
      out.write("\t\r\n");
      out.write("\tvar tabBtnView = false;\r\n");
      out.write("\tvar $winTabLeft;\r\n");
      out.write("\tvar $winTabRight;\r\n");
      out.write("\tvar windTabWidth;\r\n");
      out.write("\tvar activeIndex;\r\n");
      out.write("\tvar $liList;\r\n");
      out.write("\tvar liWidthList;\r\n");
      out.write("\tvar startTabIndex = 0;\r\n");
      out.write("\tvar endTabIndex = 0;\r\n");
      out.write("\tfunction checkTabListSize(){\r\n");
      out.write("\t\tvar tabWidth = commonUtil.getCssNum($multiList, \"width\");\r\n");
      out.write("\t\tvar windTabWidth = 0;\r\n");
      out.write("\t\tvar activeWidth = 0;\r\n");
      out.write("\t\t$liList = $multiList.find(\"li\");\r\n");
      out.write("\t\tliWidthList = new Array();\r\n");
      out.write("\t\tif($liList.length > 0){\r\n");
      out.write("\t\t\tvar $tmpObj;\r\n");
      out.write("\t\t\tvar tmpStr;\r\n");
      out.write("\t\t\tvar tmpWidth;\r\n");
      out.write("\t\t\tfor(var i=0;i<$liList.length;i++){\r\n");
      out.write("\t\t\t\t$tmpObj = $liList.eq(i);\r\n");
      out.write("\t\t\t\tif($tmpObj.hasClass(\"active\")){\r\n");
      out.write("\t\t\t\t\tactiveWidth = windTabWidth;\r\n");
      out.write("\t\t\t\t\tactiveIndex = i;\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t\ttmpWidth = commonUtil.getCssNum($tmpObj, \"width\");\r\n");
      out.write("\t\t\t\twindTabWidth += tmpWidth;\r\n");
      out.write("\t\t\t\tliWidthList[i] = tmpWidth;\r\n");
      out.write("\t\t\t}\r\n");
      out.write("\t\t\twindTabWidth -= ($liList.length-1)*7;\r\n");
      out.write("\t\t}\t\r\n");
      out.write("\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\ttabBtnView = true;\r\n");
      out.write("\t\t\tshowActiveTab();\r\n");
      out.write("\t\t}else{\r\n");
      out.write("\t\t\ttabBtnView = false;\r\n");
      out.write("\t\t\tstartTabIndex = 0;\r\n");
      out.write("\t\t\tif($liList.length == 0){\r\n");
      out.write("\t\t\t\tendTabIndex = 0;\r\n");
      out.write("\t\t\t}else{\r\n");
      out.write("\t\t\t\tendTabIndex = $liList.length -1;\r\n");
      out.write("\t\t\t}\t\t\t\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\tif(tabBtnView){\r\n");
      out.write("\t\t\tif(startTabIndex == 0){\r\n");
      out.write("\t\t\t\t$winTabLeft.hide();\r\n");
      out.write("\t\t\t\t$winTabRight.show();\r\n");
      out.write("\t\t\t}else if(endTabIndex == ($liList.length -1)){\r\n");
      out.write("\t\t\t\t$winTabLeft.show();\r\n");
      out.write("\t\t\t\t$winTabRight.hide();\r\n");
      out.write("\t\t\t}else{\r\n");
      out.write("\t\t\t\t$winTabLeft.show();\r\n");
      out.write("\t\t\t\t$winTabRight.show();\r\n");
      out.write("\t\t\t}\r\n");
      out.write("\t\t\t//$winTabLeft.show();\r\n");
      out.write("\t\t\t//$winTabRight.show();\r\n");
      out.write("\t\t\t$pathTitle.css(\"margin-right\",\"7px\");\r\n");
      out.write("\t\t}else{\r\n");
      out.write("\t\t\t$winTabLeft.hide();\r\n");
      out.write("\t\t\t$winTabRight.hide();\r\n");
      out.write("\t\t\tfor(var i=0;i<$liList.length;i++){\r\n");
      out.write("\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t}\r\n");
      out.write("\t\t\t$pathTitle.css(\"margin-right\",\"0px\");\r\n");
      out.write("\t\t}\r\n");
      out.write("\t}\r\n");
      out.write("\t\r\n");
      out.write("\tfunction showActiveTab(moveStep){\r\n");
      out.write("\t\tif($liList.length == 1){\r\n");
      out.write("\t\t\treturn;\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\tvar tabWidth = commonUtil.getCssNum($multiList, \"width\");\r\n");
      out.write("\t\tvar windTabWidth = 0;\r\n");
      out.write("\t\tif(moveStep){\r\n");
      out.write("\t\t\tif(moveStep == -1){\r\n");
      out.write("\t\t\t\tif(startTabIndex == 0){\r\n");
      out.write("\t\t\t\t\treturn;\r\n");
      out.write("\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t$liList.eq(endTabIndex).hide();\r\n");
      out.write("\t\t\t\t\tendTabIndex--;\r\n");
      out.write("\t\t\t\t\tfor(var i=endTabIndex;i>=0;i--){\r\n");
      out.write("\t\t\t\t\t\twindTabWidth += liWidthList[i];\r\n");
      out.write("\t\t\t\t\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t\t\t\t\tstartTabIndex = i;\r\n");
      out.write("\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t}else if(moveStep == 1){\r\n");
      out.write("\t\t\t\tif(endTabIndex == ($liList.length -1)){\r\n");
      out.write("\t\t\t\t\treturn;\r\n");
      out.write("\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t$liList.eq(startTabIndex).hide();\r\n");
      out.write("\t\t\t\t\tstartTabIndex++;\r\n");
      out.write("\t\t\t\t\tfor(var i=startTabIndex;i<$liList.length;i++){\r\n");
      out.write("\t\t\t\t\t\twindTabWidth += liWidthList[i];\r\n");
      out.write("\t\t\t\t\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t\t\t\t\tendTabIndex = i;\r\n");
      out.write("\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t}\r\n");
      out.write("\t\t}else{\r\n");
      out.write("\t\t\tif(activeIndex == ($liList.length -1)){\r\n");
      out.write("\t\t\t\tendTabIndex = $liList.length -1;\r\n");
      out.write("\t\t\t\tfor(var i=($liList.length -1);i>=0;i--){\r\n");
      out.write("\t\t\t\t\twindTabWidth += liWidthList[i];\r\n");
      out.write("\t\t\t\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t\t\t\tstartTabIndex = i;\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t}else if(activeIndex == 0){\r\n");
      out.write("\t\t\t\tstartTabIndex = 0;\r\n");
      out.write("\t\t\t\tfor(var i=0;i<$liList.length;i++){\r\n");
      out.write("\t\t\t\t\twindTabWidth += liWidthList[i];\r\n");
      out.write("\t\t\t\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t\t\t\tendTabIndex = i;\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t}else{\r\n");
      out.write("\t\t\t\tvar tmpActiveCheck = false;\r\n");
      out.write("\t\t\t\tendTabIndex = $liList.length -1;\r\n");
      out.write("\t\t\t\tfor(var i=($liList.length -1);i>=0;i--){\r\n");
      out.write("\t\t\t\t\twindTabWidth += liWidthList[i];\r\n");
      out.write("\t\t\t\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t\t\t\tstartTabIndex = i;\r\n");
      out.write("\t\t\t\t\t\tif(i == activeIndex){\r\n");
      out.write("\t\t\t\t\t\t\ttmpActiveCheck = true;\r\n");
      out.write("\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t}\t\t\t\t\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t\tif(!tmpActiveCheck){\r\n");
      out.write("\t\t\t\t\tvar windTabWidth = 0;\r\n");
      out.write("\t\t\t\t\tstartTabIndex = 0;\r\n");
      out.write("\t\t\t\t\tfor(var i=0;i<$liList.length;i++){\r\n");
      out.write("\t\t\t\t\t\twindTabWidth += liWidthList[i];\r\n");
      out.write("\t\t\t\t\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t\t\t\t\tendTabIndex = i;\r\n");
      out.write("\t\t\t\t\t\t\tif(i == activeIndex){\r\n");
      out.write("\t\t\t\t\t\t\t\ttmpActiveCheck = true;\r\n");
      out.write("\t\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t\tif(!tmpActiveCheck){\r\n");
      out.write("\t\t\t\t\tvar windTabWidth = 0;\r\n");
      out.write("\t\t\t\t\tendTabIndex = activeIndex;\r\n");
      out.write("\t\t\t\t\tfor(var i=($liList.length -1);i>=0;i--){\r\n");
      out.write("\t\t\t\t\t\tif(i > endTabIndex){\r\n");
      out.write("\t\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t\twindTabWidth += liWidthList[i];\r\n");
      out.write("\t\t\t\t\t\t\tif(windTabWidth > tabWidth){\r\n");
      out.write("\t\t\t\t\t\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t\t\t\t\t\t}else{\r\n");
      out.write("\t\t\t\t\t\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t\t\t\t\t\t\tstartTabIndex = i;\r\n");
      out.write("\t\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t}\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\tif(startTabIndex == 0){\r\n");
      out.write("\t\t\t$winTabLeft.hide();\r\n");
      out.write("\t\t\t$winTabRight.show();\r\n");
      out.write("\t\t}else if(endTabIndex == ($liList.length -1)){\r\n");
      out.write("\t\t\t$winTabLeft.show();\r\n");
      out.write("\t\t\t$winTabRight.hide();\r\n");
      out.write("\t\t}else{\r\n");
      out.write("\t\t\t$winTabLeft.show();\r\n");
      out.write("\t\t\t$winTabRight.show();\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\t/*\r\n");
      out.write("\t\tfor(var i=0;i<activeIndex;i++){\r\n");
      out.write("\t\t\t$liList.eq(i).hide();\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\tfor(var i=activeIndex;i<$liList.length;i++){\r\n");
      out.write("\t\t\t$liList.eq(i).show();\r\n");
      out.write("\t\t}\r\n");
      out.write("\t\t*/\r\n");
      out.write("\t}\r\n");
      out.write("\t\r\n");
      out.write("\tfunction tabLeft(){\r\n");
      out.write("\t\tif(activeIndex > 0){\r\n");
      out.write("\t\t\tactiveIndex --;\r\n");
      out.write("\t\t\tshowActiveTab(-1);\r\n");
      out.write("\t\t}\r\n");
      out.write("\t}\r\n");
      out.write("\t\r\n");
      out.write("\tfunction tabRight(){\r\n");
      out.write("\t\tif(activeIndex < $liList.length-1){\r\n");
      out.write("\t\t\tactiveIndex ++;\r\n");
      out.write("\t\t\tshowActiveTab(1);\r\n");
      out.write("\t\t}\r\n");
      out.write("\t}\r\n");
      out.write("</script>\r\n");
      out.write("</head>\r\n");
      out.write("<body class=\"content_header\">\r\n");
      out.write("<button type=\"button\" class=\"lnbCloser\" onclick=\"sizeToggle(this)\"><img src=\"/common");
      out.print(theme);
      out.write("/images/ico_lnb_closer.png\" alt=\"LNB Close\" /></button>\r\n");
      out.write("<div class=\"tab type1\" id=\"windowTabs\">\r\n");
      out.write("\t<div class=\"menu-info\" id=\"pathTitle\">\r\n");
      out.write("\t\t<div class=\"all_close\" id=\"allClose\">\r\n");
      out.write("\t\t\t<img src=\"/common");
      out.print(theme);
      out.write("/images/btn_closeAll.png\" alt=\"close\" onClick=\"closeAll()\" />\t\t\t\r\n");
      out.write("\t\t</div>\r\n");
      out.write("\t\t<div class=\"tab_list\" id=\"tabList\">\r\n");
      out.write("\t\t\t<img src=\"/common");
      out.print(theme);
      out.write("/images/ico_lnb_closer.png\" alt=\"left\"  id=\"winTabLeft\" onClick=\"tabLeft()\" style=\"display:none\" />\r\n");
      out.write("\t\t\t<img src=\"/common");
      out.print(theme);
      out.write("/images/ico_lnb_closel.png\" alt=\"right\" id=\"winTabRight\" onClick=\"tabRight()\" style=\"display:none;\" />\r\n");
      out.write("\t\t</div>\r\n");
      out.write("\t</div>\r\n");
      out.write("\t<ul id=\"multiList\">\r\n");
      out.write("\t\t<li><div class=\"tab_right\"><span class=\"txt_wrap\">Sample</span><img src=\"/common");
      out.print(theme);
      out.write("/images/btn_null.png\" alt=\"close\" /></div></li>\r\n");
      out.write("\t</ul>\r\n");
      out.write("</div>\r\n");
      out.write("</body>\r\n");
      out.write("</html>");
    } catch (java.lang.Throwable t) {
      if (!(t instanceof javax.servlet.jsp.SkipPageException)){
        out = _jspx_out;
        if (out != null && out.getBufferSize() != 0)
          try {
            if (response.isCommitted()) {
              out.flush();
            } else {
              out.clearBuffer();
            }
          } catch (java.io.IOException e) {}
        if (_jspx_page_context != null) _jspx_page_context.handlePageException(t);
        else throw new ServletException(t);
      }
    } finally {
      _jspxFactory.releasePageContext(_jspx_page_context);
    }
  }
}
