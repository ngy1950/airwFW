...
<link rel="stylesheet" type="text/css" href="/common<%=theme%>/css/wintab.css">
...
<script type="text/javascript">
	...
	*/
	...
	function closeWindow(menuId, allType){
		if(!allType && !window.top.checkWindowModify(menuId)){
			return;
		}
	...
	function checkTabListSize(){
		...
		if($liList.length > 0){
			...
			for(var i=0;i<$liList.length;i++){
				...
				windTabWidth += tmpWidth;
				liWidthList[i] = tmpWidth;
			}
		...
		if(tabBtnView){
			if(startTabIndex == 0){
				$winTabLeft.hide();
				$winTabRight.show();
			}else if(endTabIndex == ($liList.length -1)){
				$winTabLeft.show();
				$winTabRight.hide();
			}else{
				$winTabLeft.show();
	...
	function showActiveTab(moveStep){
		if($liList.length == 1){
			return;
		}
		var tabWidth = commonUtil.getCssNum($multiList, "width");
		...
		if(moveStep){
			if(moveStep == -1){
				if(startTabIndex == 0){
					...
					$liList.eq(endTabIndex).hide();
					endTabIndex--;
					for(var i=endTabIndex;i>=0;i--){
						windTabWidth += liWidthList[i];
						if(windTabWidth > tabWidth){
							$liList.eq(i).hide();
						}else{
							$liList.eq(i).show();
				...
				if(endTabIndex == ($liList.length -1)){
					return;
				}else{
					$liList.eq(startTabIndex).hide();
					startTabIndex++;
					for(var i=startTabIndex;i<$liList.length;i++){
						windTabWidth += liWidthList[i];
						if(windTabWidth > tabWidth){
							$liList.eq(i).hide();
						}else{
...
<button type="button" class="lnbCloser" onclick="sizeToggle(this)"><img src="/common<%=theme%>/images/ico_lnb_closer.png" alt="LNB Close" /></button>
<div class="tab type1" id="windowTabs">
	<div class="menu-info" id="pathTitle">
		<div class="all_close" id="allClose">
			<img src="/common<%=theme%>/images/btn_closeAll.png" alt="close" onClick="closeAll()" />			
		</div>
		<div class="tab_list" id="tabList">
			<img src="/common<%=theme%>/images/ico_lnb_closer.png" alt="left"  id="winTabLeft" onClick="tabLeft()" style="display:none" />
			<img src="/common<%=theme%>/images/ico_lnb_closel.png" alt="right" id="winTabRight" onClick="tabRight()" style="display:none;" />
		</div>
	</div>
	<ul id="multiList">
		<li><div class="tab_right"><span class="txt_wrap">Sample</span><img src="/common<%=theme%>/images/btn_null.png" alt="close" /></div></li>
	</ul>
</div>
</body>
</html>