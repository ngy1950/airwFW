...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$menuSearch.autocomplete({
			...
			*/
			select: function( event, ui ) {
				...
				var menuId = ui.item.value;
				if(menuId.indexOf("[") != -1){
					menuId = menuId.substring(menuId.indexOf("[")+1,menuId.indexOf("]"));
				}
				menuSearch(menuId);
		...
		$menuSearch.on('focus', function(e) { 
			$menuSearch.val("");
		});
	});
...
<div class="lnb">
	<div class="icons">
		<input type="text" id="menuSearch" size="14" onkeypress="commonUtil.enterKeyCheck(event,'menuSearch()')" value="검색어를 입력하세요"/>
		<div class="rightArea">
			<!-- button class="button type1 first" type="button"><img src="/common<%=theme%>/images/ico_left1.png" alt=""></button-->
			<button class="button type1 second" type="button" onclick="fmenuInsert()"><img src="/common/theme/gsfresh/images/login/ico_left2.png" alt="">즐겨찾기</button>
			<button class="button type1" type="button" onclick="window.top.refreshPage()"><img src="/common/theme/gsfresh/images/login/ico_left3.png" alt="">새로고침</button>		
		</div>
	</div>
...
%>		
	autocompleteList.push("<%=map.getString("LBLTXL")%>");
<% 		
		}
	}
%>
