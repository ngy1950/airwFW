  ...
  //헤더 조회 
	...
	function saveData(){
		if( gridList.validationCheck("gridHeadList", "select") ){
			...
			if(boxmak == "V"){
				commonUtil.msgBox("VALID_M0508"); //이미 확정된 데이터 입니다.
				return;				
			}
			...
			if( json2 && json2.data ){
				if(json2.data != "OK"){
					if(json2.data["RESULT"] == "1"){
						commonUtil.msgBox("VALID_M0522"); //박스최적화를 수행한 후에 오더확정 가능합니다
						return;
					}else if(json2.data["RESULT"] == "2"){
						commonUtil.msgBox("VALID_M0501"); //주문과 박스최적화 결과가 다릅니다.\n박스최적화 재실행 후, 오더확정이 가능합니다.
						return;
					}
				}
			}
	...
	function saveData2(){
		...
		if( json2 && json2.data ){
			if(json2.data != "OK"){
				if(json2.data["RESULT"] == "1"){
					commonUtil.msgBox("VALID_M0502"); //체적이 미등록된 상품이 있습니다.\n체적을 모두 등록하여 주세요.
					return;
				}else if(json2.data["RESULT"] == "2"){
					commonUtil.msgBox("VALID_M0503"); //피킹로케이션이 미지정된 상품이 존재합니다.\n피킹로케이션을 모두 등록하여 주세요.
					return;
				}else if(json2.data["RESULT"] == "3"){
					commonUtil.msgBox("VALID_M0504"); //주문수량과 일치 하지 않습니다.
					return;
				}else if(json2.data["RESULT"] == "4"){
					commonUtil.msgBox("VALID_M0505"); //물류분류가 없는 상품이 존재합니다. 물류분류를 모두 지정하여 주세요
					return;
				}else if(json2.data["RESULT"] == "5"){
					commonUtil.msgBox("VALID_M0521"); //선택된 차수의 오더확정 정보가 변경 되었습니다.\n재조회 하시기 바랍니다.
					return;
				}
			}
		}
	...
	function saveData3(){
		...
		if(dangood == 'Y'){
			...
			if(!commonUtil.msgConfirm("피킹지시를 하시겠습니까?")){//출하지시를 하시겠습니까?
	            return;
	        }
		}
		...
		if( gridList.validationCheck("gridHeadList", "select") ){
			...
			if(json2 && json2.data){
				if(parseInt(json2.data["VAL05"]) == 0 && boxmak == 'V'){
					...
					commonUtil.msgBox("VALID_M0525");//확정된 오더 정보가 아닙니다.
					return;
				}
			}
			...
			if(json3 && json3.data){
				if(json3.data["CHK"] == "N"){
					commonUtil.msgBox("VALID_M0526"); //출하지시된 정보로 오더확정 취소를 할수 없습니다.
					return;
				}
			}
				...
				<div class="tabs">
					...
					<div id="tabs1-1">
						<div class="section type1">
							<div class="colInfo">
								<ul>
									<li>*eBOS 주문 송신 건수와 WMS 수신 건수 불일치시 박스체적시뮬레이션 진행 불가</li>
									<li>*eBOS 주문과 오더라인 수신 건수와 TMS 배차 계획 완료 수신 건수 불일치시 피킹지시 불가</li>
								</ul>
							</div>
									...
									<table>
										<tbody id="gridHeadList">
											<tr CGRow="true">
												...
												<td GH="100 배차현황" GCol="btn2, " GB="Recommend icon_detail BTN_RECOMD"></td>
											</tr>
										</tbody>
									</table>
								</div>
