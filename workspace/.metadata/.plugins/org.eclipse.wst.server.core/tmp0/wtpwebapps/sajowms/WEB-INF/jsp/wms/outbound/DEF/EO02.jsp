...
<script type="text/javascript">
	...
	function searchList(btnName){
		if( validate.check("searchArea") ){
			...
			gridList.setReadOnly("gridList2", true, ["SKUCLS", "PRCQTY"]);
	...
	function gridListEventDataBindEnd(gridId, dataLength){
		if( gridId == "gridList1" && dataLength == 0 ){
			...
			var searchArea = inputList.setRangeMultiParam("searchArea");
			var docuty = searchArea.get("DOCUTY");
	...
	function saveData(){
		if( gridList.validationCheck("gridList2", "modify") ){
			var headRowNum = gridList.getFocusRowNum("gridList1");
			var head = gridList.getRowData("gridList1", headRowNum);
			...
			if( head.get("CREDAT") != todayDate ){
				commonUtil.msgBox("OUT_M0297"); //당일 반품요청건만 취소 가능합니다.
				return false;
			}
			
			if( head.get("DOCUTY") == "442" ){
				alert("센터반품취소는 취소할 수 없습니다.");
				return false;
			}
	...
	function linkPopCloseEvent(closeParam){
		if( closeParam.get("searchCode") == "SHSKUMA" ){
			...
			netUtil.send({
				url : "/wms/etcOutbound/json/saveEO02.data",
				param : param,
	...
	function gridListEventColValueChange(gridId, rowNum, colName, colValue){
		...
		if ( colName == "PRCQTY" ){//수량
			...
			if( NumColValue == 0 ){
				gridList.setColValue(gridId, rowNum, "PRCQTY", rowData.get("ORGPRCQTY")); //기존반품수량
				commonUtil.msgBox("OUT_M0290"); //반품수량은 0보다 커야합니다.
				return false;
			}
			
			if( Number(rowData.get("ORGPRCQTY")) < NumColValue ){
				gridList.setColValue(gridId, rowNum, "PRCQTY", rowData.get("ORGPRCQTY")); //기존반품수량
				commonUtil.msgBox("OUT_M0296"); //반품취소수량은 기존 반품수량보다 클 수 없습니다.
				return false;
			}
	...
	function searchHelpEventOpenBefore(searchCode, multyType, $inputObj, rowNum){
		...
		if(searchCode == "SHBZPTN"){
			...
			if( search.get("DOCUTY") == "441" ){ //협력업체 반품
				param.put("PTNRTY","V");
				
			}else if( search.get("DOCUTY") == "442" ){ //센터반품
				param.put("PTNRTY","S");
				param.put("PTNG04", "V");
								...
								<div class="tableBody">
									<table>
										...
										<tr CGRow="true">
												...
												<td GH="100 STD_DPTNEO"    GCol="text,PTNRKY"></td><!-- 반품처코드 -->
												<td GH="100 STD_DPTNME"    GCol="text,PTNRNM"></td><!-- 반품처명 -->
								...
								<div class="tableBody">
									<table>
										...
										<tr CGRow="true">
												...
												<td GH="80 STD_CNLQTY"    GCol="input,PRCQTY"  GF="N 7" validate="required gt(0),MASTER_M4002"></td>
