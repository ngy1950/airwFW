...
<%
	String rcode = "";
	if(menuId.equals("TK32")){
		rcode = "331";
	}else if(menuId.equals("TK31")){
		rcode = "361";
	}else if(menuId.equals("TK30")){
		rcode = "360";
	}else if(menuId.equals("TK36")){
		rcode = "399";
	}else if(menuId.equals("TK39")){
		rcode = "380";
	}
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if(menuky != "TK36" && menuky != "TK30" && menuky != "TK39"){
			rdata.put(configData.INPUT_RANGE_SINGLE_DATA, userInfoData.LOCA);
		}
		inputList.setRangeData("STKKY.LOCAKY", configData.INPUT_RANGE_TYPE_SINGLE, [rdata]);
		
// 		rdata.put(configData.INPUT_RANGE_SINGLE_DATA, userInfoData.DEPART);
// 		inputList.setRangeData("STKKY.LOTA02", configData.INPUT_RANGE_TYPE_SINGLE, [rdata]);
		
		
		if(menuky == "TK36" || menuky == "TK39"){
			$("#ptnrCombo").hide();
			$("#zoneInput").hide();
		}else if(menuky == "TK30"){
			$("#zoneInput").hide();
		...
		// reasonCombo Change
		var $comboObj = $("#arsnadjCombo");
		var comboData = $('#TASOTY').val();
		inputList.reloadCombo($comboObj, configData.INPUT_REASON_COMBO, comboData);
		gridList.setComboOption("gridList", "RSNCOD", configData.INPUT_REASON_COMBO, comboData);
		
		$('#TASOTY').change(function(){
			$comboObj = $("#arsnadjCombo");
			comboData = $('#TASOTY').val();
			inputList.reloadCombo($comboObj, configData.INPUT_REASON_COMBO, comboData);
			gridList.setComboOption("gridList", "RSNCOD", configData.INPUT_REASON_COMBO, comboData);
		});
				...
				<tbody>
					...
					<tr>
						...
						<td GCol="select,AREAKY" validate="required,INV_M0054">
							<select Combo="WmsTask,TASOTYCD" name="TASOTY" id="TASOTY">
							</select>
						</td>
					</tr>
					
					<tr id="ptnrCombo">
						<th CL="STD_PTNRKY_TASK">수신부서</th>
						<td GCol="select,PTNRKY">
							<select Combo="WmsTask,BZPTNCOMBO" name="PTNRKY">
							</select>
						</td>
					</tr>
				</tbody>
			...
			<table class="table type1">
				...
				<tbody>
					...
					<tr id="zoneInput">
						...
						<td>
<!-- 							<input type="text" name="STKKY.ZONEKY"/> -->
...
<!-- 							<input type="text" name="STKKY.LOCAKY" UIInput="R,SHLOCMA" /> -->
				...
				<tbody>
					...
					<!-- <tr>
						...
						<td GCol="select,LOTA02">
							<select Combo="WmsTask,LOTA02COMBO" name="LOTA02">
								<option value=""> </option>
							</select>
						</td>
					</tr> -->
		...
		<!-- 
		<div  id="to05combo" class="searchInBox">
			<h2 class="tit" CL="STD_DOCUMENTINFO">문서정보</h2>
			<table class="table type1">
				<colgroup>
					<col width="100" />
					<col />
				...
				<tbody>
					<tr>
						<th CL="STD_RECVKY">입하문서번호</th>
						<td>
							<input type="text" name="RECVKY" />
						</td>
					</tr>
					<tr>
						<th CL="STD_SHPOKY">출하문서번호</th>
						<td>
							<input type="text" name="SHPOKY"  />
						</td>
					</tr>
					<tr>
						<th CL="STD_SEBELN">ECMS 주문번호</th>
						<td>
							<input type="text" name="SEBELN" />
						</td>
					</tr>
				</tbody>
			</table>
									...
									<table>
										...
										<thead>
											<tr>
												...
												<th CL='STD_PTNRKY_TASK'></th>
												<th CL='STD_PTNRNM_TASK'></th>
												<th CL='STD_DOCTXT'></th>
												<th CL='STD_CREDAT'></th>
												<th CL='STD_CRETIM'></th>
												<th CL='STD_CREUSR'></th>
						...
						<div class="section type1">
							<table class="util">
								<tr>
									<th CL="STD_RSNCOD"></th>
									<td>
<!-- 										<select COMBO="WmsTask,RSNCOMBO" id="rsnadjCombo"> -->
											<select ReasonCombo="<%= rcode%>" id="rsnadjCombo">
											<option value=""> </option>
										</select>
									</td>
									<td>
									...
									<table>
										...
										<tbody id="gridList">
											<tr CGRow="true">
												...
												<!-- <td GCol="select,LOTA02">
													<select Combo="WmsTask,LOTA02COMBO">
														<option value=""> </option>
													</select>
												</td> -->
