...
<script type="text/javascript" src="/common/js/worker-ajax.js?20170622"></script>
<script type="text/javascript" src="/common/js/bigdata.js"></script>
<script type="text/javascript" src="/common/js/dataRequest.js?20170622"></script>
<script type="text/javascript" src="/common/js/grid.js?20170828"></script>
<script type="text/javascript" src="/common/js/layoutSave.js?20170622"></script>
<script type="text/javascript" src="/common/js/validateUtil.js?20170622"></script>
<script type="text/javascript" src="/common/js/page.js?20170622"></script>
<script>
//탭 인덱스 변경
(function() {
	$(function() {
			...
			for(var prop in openerParam.map){
				if(inputList.rangeMap.containsKey(prop)){
					...
					if(inputList.rangeMap.containsKey(tmpProp)){
						...
						//2019.12.16 김호철 >> 멀티콤보값 넘어왔을 때...
						var paramVal = openerParam.get(prop);
						if(paramVal.indexOf("'") == -1) {
							inputList.setRangeData(tmpProp, configData.INPUT_RANGE_RANGE_FROM, openerParam.get(prop));
						} else {
							paramVal = commonUtil.replaceAll(paramVal, "'", "");
							var paramSplList = paramVal.split(",");
							var rangeList = new Array();
							
							for(var i=0;i<paramSplList.length;i++){
								var paramDataVal = paramSplList[i];
								
								var pMap = new DataMap();
								
								pMap.put("OPER", "E");
								pMap.put("LOGICAL", "OR");
								pMap.put("DATA", paramDataVal);
								rangeList.push(pMap);
							}
							inputList.setRangeData(tmpProp, configData.INPUT_RANGE_TYPE_SINGLE, rangeList);
						}
					}				
				}
			}
