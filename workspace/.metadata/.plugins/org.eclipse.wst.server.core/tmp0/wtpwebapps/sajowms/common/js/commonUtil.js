...
CommonUtil.prototype = {
	...
	datemath : function (obj1,obj2,diffnum){
		var f = obj1.val().replace(/\./g,'');
		var t = obj2.val().replace(/\./g,'');
		
	    var from = new Date(f.substring(0,4)+"-"+f.substring(4,6)+"-"+f.substring(6,8));
	    var to = new Date(t.substring(0,4)+"-"+t.substring(4,6)+"-"+t.substring(6,8));
	    ...
	    if(day > diffnum){
	    	var prm = diffnum + "";
	    	this.msgBox("VALIDATE_datediffchk",prm);
	    	obj1.focus();
			return false;
	    }
	    return true;
	},
	roleCheck : function(progid,userid,saveRole,printRole,msgYn) {
		var roleParm = new DataMap();
		roleParm.put("PROGID"     , progid);
		roleParm.put("USERID"     , userid);
		roleParm.put("SAVE_CLICK" , saveRole);	//권한체크 : Y
		roleParm.put("PRINT_CLICK", printRole);	//권한체크 : Y
		
		var json = netUtil.sendData({
			module : "WmsCommon",
			command : "ROLPG_ROLECHK",
			sendType : "map",
			param : roleParm
		});
		...
		if( json && json.data ){
			if(json.data["CNT"] > 0) {
				if(msgYn != "N") {
					var msgtxt;
					if(printRole == "Y") {msgtxt = "출력";}
					if(saveRole == "Y")  {msgtxt = "저장";}
					alert("[" + progid + "]의 " + msgtxt + "의 권한이 없습니다.");
				}
				return false;
			}
		} else {
