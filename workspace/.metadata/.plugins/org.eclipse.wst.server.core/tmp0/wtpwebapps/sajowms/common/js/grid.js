GridBox = function(id) {
	...
	this.$gridLayout = this.$box.parents("."+configData.GRID_BOX_CLASS);
	this.$gridBody = this.$gridLayout.find("."+configData.GRID_BODY_CLASS);
	if(this.$box.find("["+configData.GRID_HEAD+"]").length > 0){
		theme.createGridHead(this);
	}	
	this.$gridHead = this.$gridLayout.find("."+configData.GRID_HEAD_CLASS);
	this.headCells = new Array();
	this.headRowCount = 0;
	this.$gridBottom = this.$gridLayout.find("."+configData.GRID_BOTTOM_CLASS);
	this.$girdViewCount;
	...
	this.startRowNum = 0;//viewRowNum
	this.endRowNum = 0;//viewRowNum
...
GridBox.prototype = {
	...
	colFix : function() {
		...
		this.$colFix.addClass(configData.GRID_COLFIXBODY_CLASS);
		...
		this.$headColFix.addClass(configData.GRID_COLFIXHEAD_CLASS);
	...
	changeSumColNum : function(colName, numberType){
		...
		this.$gridHead.find(this.headTagName+" > "+this.headRowTagName+":eq(1)").find("["+configData.GRID_COL_NAME+"="+colName+"]").text(totNum);
	},
	...
	setHead : function(){
		...
		if(this.headColDragType && this.draggInitType){
			//this.setHeadColDrag(this.$gridHead.find("."+configData.GRID_LAYOUT_HEAD_COL_LEFT_CLASS));
			this.setHeadColDrag(this.$gridHead.find("."+configData.GRID_LAYOUT_HEAD_COL_RIGHT_CLASS));
		}
		...
		//if(this.sortType){
			this.$gridHead.find(this.headTagName+" > "+this.headRowTagName).dblclick(function(event){
				var $obj = jQuery(event.target);
				if($obj.hasClass(configData.GRID_LAYOUT_HEAD_COL_RIGHT_CLASS)){
					return;
				}
	...
	setHeadTdHtml : function($thObj) {
		...
		if(this.headColDragType){
			...
			var tmpTable = jQuery("<table class='thInTable' style='width:100%;' title=\""+$thObj.attr(configData.GRID_COL_VALUE)+"\"><tr><td style='width:10px;'></td><td "+configData.GRID_COL_NAME+"='"+colName+"' >"+colText+"</td><td style='width:3px;' class='"+configData.GRID_LAYOUT_HEAD_COL_RIGHT_CLASS+"'></td></tr></table>");
			if($thObj.attr(configData.GRID_BTN_CHECK_ATT)){
				tmpTable.find("."+configData.GRID_LAYOUT_HEAD_COL_RIGHT_CLASS).removeClass(configData.GRID_LAYOUT_HEAD_COL_RIGHT_CLASS);
			}else{
				tmpTable.find("."+configData.GRID_LAYOUT_HEAD_COL_RIGHT_CLASS).attr(configData.GRID_COL_NAME, colName);
			}
	...
	startHeadColWidth : function($dragObj, ui) {		
		...
		if($dragObj.attr(configData.GRID_LAYOUT_HEAD_COL_LEFT_CLASS)){
	...
	setHeadColWidthName : function(colName) {
		...
		if(this.draggInitType){
			this.setHeadColDrag($headTd.find("."+configData.GRID_LAYOUT_HEAD_COL_RIGHT_CLASS));
		}else{
	...
	reloadView : function(focusType, startType){
		...
		this.$box.find(
				"["+configData.GRID_ROW+"]").remove();
		
		if(this.scrollType){			
			var startRow = 0;
			var endRow = this.viewRowCount;
			this.setView(startRow, endRow);
	...
	*/
	...
	copyDataView : function($sColObj, copyData){
		...
		//2020.02.04 이범준 클립보드로 복사할 데이터수
		this.copyDataLen = dataList.length;
		...
		for(var i=0;i<dataList.length;i++){
			...
			if(rowNum != undefined && dataList[i]){
				...
				if(this.readOnlyRowMap && this.readOnlyRowMap.get(rowNum)){
					this.copyDataLen = this.copyDataLen - 1;
					continue;
				}
	...
	multiDataView : function(startRowNum, colName, dataList){
		...
		var tmpMaxCount = this.viewDataList.length - sRowViewNum;
		if(this.addType && this.autoNewRowType && tmpMaxCount < dataList.length){
			tmpMaxCount = dataList.length - tmpMaxCount;
	...
	clickEvent : function($obj, event){
		...
		if(tmpTagName == "BUTTON"){
			...
			if($obj.attr("class") == configData.GRID_COL_TYPE_FILE_TYPE){
				...
				gridList.fileChooser(this.id, rowNum, colName, $rowObj, $obj, tmpFileType, true); // 2019.03.20 jw : $rowObj Parameter Added
			}
		...
		if(colType){						
			...
			*/
				...
				if($obj.hasClass("btn_delY")){
					gridList.fileChooser(this.id, rowNum, colName, $rowObj, $obj, tmpFileType); // 2019.03.20 jw : $rowObj Parameter Added
				}else{
					gridList.fileChooser(this.id, rowNum, colName, $rowObj, $obj, tmpFileType, true); // 2019.03.20 jw : $rowObj Parameter Added
				}
			}else if(colType == configData.GRID_COL_TYPE_FILE_DOWNLOAD){ // 2019.03.20 jw : Grid Data Type FileDownload Added
				gridList.fileChooser(this.id, rowNum, colName, $rowObj, $obj, tmpFileType);
		...
		if($dataRow.length){
			...
			$dataRow.find("["+ configData.GRID_COL +"]").each(function(i, findElement){
				...
				if(attList[0] == configData.GRID_COL_TYPE_ROWNUM){
					...
					var btnHtml = "<button type='button' class='"+configData.BUTTON_CLASS+" "+configData.GRID_COL_TYPE_BTN_CLASS+valTxt+"' "
								+ " onclick='gridList.btnClick(\""+gridBox.id+"\",this)' >"
								+ "<span class='"+configData.BUTTON_ICON_CLASS+" "+iconClass+"'>Icon</span>"
								+ "<span class='"+configData.BUTTON_TEXT_CLASS+"'>"+labelTxt+"</span>"
								+ "</button>";
					...
					/* 김호영 2019-04-04 */
						...
						$obj.addClass("gridColTextCenter");
						$obj.append($btnObj);
						/* 김호영 2019-04-04 */
				}else if(attList[0] == configData.GRID_COL_TYPE_ICON){
					/* 이범준 2019-04-05 : Icon 이미지 넣기*/
					$obj.addClass(configData.GRID_COL_DATA_NAME_READONLY_CLASS_HEAD+attList[1] + " gridColText");
					var bCode = $obj.attr(configData.GRID_COL_BUTTON).split(" ");
					var iconType = bCode[0];
				    var icon = '<div class="gridIcon-center '+attList[1]+'">'
							 + '<span class="'+iconType+'">Icon</span>'
							 + '</div>';
					var $iconObj = jQuery(icon);
					
					if(gridBox.gridIconMap.get(gridBox.id)){
						gridBox.gridIconMap.get(gridBox.id).put(attList[1],icon);
					}else{
						var iconMap = new DataMap();
						iconMap.put(attList[1],icon);
						gridBox.gridIconMap.put(gridBox.id,iconMap);
					}
					$obj.append($iconObj);
			}else if(attList[0] == configData.GRID_COL_TYPE_INPUT){
					...
					if(attList[0] == configData.GRID_COL_TYPE_FILE){ // 2019.03.20 jw : Grid Data Type FileDownload Added
						tmpHtml += "<button type='button' class='fileType' title='file upload'><img src='/common/images/"+configData.BUTTON_ICON.get(configData.BUTTON_TYPE_GRID_FILE)+".png'></button>";
					}
		...
		*/
		...
		if(this.defaultRowStatus == configData.GRID_ROW_STATE_INSERT || this.defaultRowStatus == configData.GRID_ROW_STATE_UPDATE){
			...
			for(var i=appendStart;i<=appendEnd;i++){
				var tmpRowNum = this.getRowNum(i);
				this.setRowState(tmpRowNum, this.defaultRowStatus);
			}
		}
	...
	sortMulti : function(preCount, colName, ascType, viewType) {
		...
		var rowData;
		/**20200424 전상욱 멀티소팅 조회시 오류 부분 수정*/
		var cIndex = (this.sortColNumList.length-1) > (preCount+1) ? (preCount+1) : (this.sortColNumList.length-1);	
		var dIndex = this.sortColNumList[cIndex];
	...
	setHeight : function() {
		...
		if(this.bodyBoxHeight == 0){
			var $tabBox = this.$bodyBox.parents("."+configData.GRID_TAB_BOX_CLASS);
			if($tabBox.length > 0){
	...
	scrollEnd : function(event, appendRowType) {
		...
		//2020.01.22 이범준 scroll 시 마지막 row가 잘리는 현상  : floor -> ceil로 수정  
		var endRow = Math.ceil((nowScroll+this.bodyBoxHeight)/this.rowHeight)-1;
		...
		for(var i=startRow;i<=endRow;i++){
			...
			*/
			...
			for(var j=0;j<this.cols.length;j++){
				...
				if(iconClassRemove){
					var iconMap = this.gridIconMap.get(this.id);
					var iconClassText = iconMap.get(colName);
	...
	setColValue : function(rowNum, colName, colValue, eventType, viewType, $rowObj, $colObj, checkStart) {
		...
		if(commonUtil.isJObjNotEmpty($rowObj)){
			...
			if($obj){	
				...
				if(this.colFixType){
					var $chekColfix = $obj.parents("."+configData.GRID_COLFIXBODY_CLASS);
...
GridList.prototype = {
	clipDataCopy : function(gridId, $obj) {
		...
		jQuery("#ClipSavePopText").val("");
    	showClipSave();
	},
	clipDataView : function() {
		var tmpTxt = jQuery("#ClipSavePopText").val();
		if(tmpTxt){
	...
	*/
	...
	shortKeyDown : function($obj, code){
		if($obj.parents("."+configData.GRID_BODY_CLASS).length > 0){
			return;
		}
		if($obj.parents("."+configData.GRID_HEAD_CLASS).length > 0){
			return;
		}
	...
	fileChooser : function(gridId, rowNum, colName, $rowObj, $colObj, fileType, openFilePop) { // 2019.03.20 jw : $rowObj Parameter Added
		...
		if(this.mFileType){
			...
			if(this.fileUUID.length == 36){
				...
				showMFileUpload(this.fileUUID, list, fileType);
			}else{
				showMFileUpload();
			}
					...
					if(json && json.data){
						...
						if(json.data["UPTBYN"] == "Y"){
							if(openFilePop){
								...
								if(!commonUtil.msgConfirm("COMMON_M0503",[json.data["NAME"]])){
									return;
								}
