InputObj = function($obj) {
	this.$obj = $obj.attr("autocomplete","off");//생성 옵션
	...
	setInput : function() {
		...
		if(this.options[0] == configData.INPUT_SEARCH){
			...
			if(this.options[0] == configData.INPUT_RANGE || this.options[0] == configData.INPUT_SHORT_RANGE){
				var $rangeBtn = theme.getInputRangeBtn();
				...
				if(rangeCount > 0){
					for(var j=0;j<this.rangeData.length;j++){
						...
						if(this.formatList){
							...
							// 2020.10.14 khc : 일자일 경우에 두 일자 사이의 기간 체크
							try {
								if(this.formatList[0] == configData.INPUT_FORMAT_CALENDER 
										&& this.inputAtt == "B"
										&& this.$obj.attr(configData.VALIDATION_DATE_MAX_DIFF).length > 0) {
									
									var diffNum = 0;
									if(isNaN(this.$obj.attr(configData.VALIDATION_DATE_MAX_DIFF))) {
										//문자로 넘어오면 옵션에서 찾는다
										switch (this.$obj.attr(configData.VALIDATION_DATE_MAX_DIFF)) {
						                	case 'M1':	diffNum = configData.VALIDATION_DATE_MAX_DIFF_M1;	break;
						                	case 'M2':	diffNum = configData.VALIDATION_DATE_MAX_DIFF_M2;	break;
						                	case 'M3':	diffNum = configData.VALIDATION_DATE_MAX_DIFF_M3;	break;
						                	case 'M4':	diffNum = configData.VALIDATION_DATE_MAX_DIFF_M4;	break;
						                	case 'M6':	diffNum = configData.VALIDATION_DATE_MAX_DIFF_M6;	break;
						                	case 'Y1':	diffNum = configData.VALIDATION_DATE_MAX_DIFF_Y1;	break;
						                	default:	diffNum = 0;	break;
										}
									} else {
										//숫자로 넘어오면 숫자로 계산
										diffNum = this.$obj.attr(configData.VALIDATION_DATE_MAX_DIFF);
									}
									
									if(diffNum > 0) {
										//일자 사이의 간격 구하기
										var dtFr = dataMap.get(configData.INPUT_RANGE_RANGE_FROM);
										var dtTo = dataMap.get(configData.INPUT_RANGE_RANGE_TO);
										
										dtFr = dtFr.substring(0,4) + "-" + dtFr.substring(4,6) + "-" + dtFr.substring(6,8);
										dtTo = dtTo.substring(0,4) + "-" + dtTo.substring(4,6) + "-" + dtTo.substring(6,8);
										
										var diffDate_fr = dtFr instanceof Date ? dtFr :new Date(dtFr);
									    var diffDate_to = dtTo instanceof Date ? dtTo :new Date(dtTo);
									    
									    diffDate_fr = new Date(diffDate_fr.getFullYear(), diffDate_fr.getMonth()+1, diffDate_fr.getDate());
									    diffDate_to = new Date(diffDate_to.getFullYear(), diffDate_to.getMonth()+1, diffDate_to.getDate());
									    
									    var diff = Math.abs(diffDate_to.getTime() - diffDate_fr.getTime());
									    diff = Math.ceil(diff / (1000 * 3600 * 24));
									    
									    if(!isNaN(diff) && diff > diffNum) {
								    		checkResult = false;
											this.$from.focus();
											commonUtil.msgBox("VALIDATE_datediffchk", diffNum);
											break;
									    }
									}
								}
...
InputList.prototype = {
	...
	copyRangeParam : function(param, origName, targetName) {
		if(param.containsKey(configData.INPUT_RNAGE_DATA_PARAM)){
			var rangeData = param.get(configData.INPUT_RNAGE_DATA_PARAM).get(origName+"_"+configData.INPUT_RANGE_TYPE_RANGE);
	...
	setCombo : function(areaId) {
		...
		for(var i=0;i<$comboList.length;i++){
			...
			var optionHtml;
			if(this.comboMap.containsKey(commonCode)){
		...
		for(var i=0;i<$comboList.length;i++){
			...
			var codeViewType = this.getComboCodeViewAtt($comboList.eq(i));
			var optionHtml;
			if(this.comboMap.containsKey(reasonCode)){
		...
		for(var i=0;i<$comboList.length;i++){
			...
			var optionHtml;
			//김호철 : 동일한 콤보id시, 파라메터 미setting문제로 주석처리함.
//			if(this.comboMap.containsKey(codeKey)){
