...
<%
	...
	String passwd = detectXSSEx(encrypt("SHA1", request.getParameter("passwd")));
	...
	if(id.equals(u_id)){
		if(passwd.equals(u_pass)){
			...
			result_sc = "<script>alert(pe_bD.pe_ph); history.back();</script>";
		}
	}
	else{
		result_sc = "<script>alert(pe_bD.pe_wF); history.back();</script>";
	}
