...
<%
	...
	String userAddMenuList [] = detectXSSEx(request.getParameter("UserAddMenu")).split("\\|\\|");
	...
	if(check){
		result_sc = "<script type='text/javascript' language='javascript'>alert(pe_bD.pe_pk); window.document.location.href='manager_setting.jsp?Tab=" + detectXSSEx(request.getParameter("Tab")) + "';</script>";
	}
	else{
		result_sc = "<script>alert(pe_bD.pe_ll+'" + fileName + "');  history.back();</script> ";
	}
