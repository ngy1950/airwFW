...
<%
	String isInitHtml = request.getParameter("isInitHtml"); 
	if(isInitHtml != null) {
		String saveDirPath = request.getRealPath("/");
		saveDirPath += ("UploadDir" + File.separator);
		File isSecondFile = new File(saveDirPath + "isSecondFile.session"); 
		if(isSecondFile.exists() && isSecondFile.isFile()) { 
			isSecondFile.delete(); 
		}
	}
	else { 
		...
		try { 
			...
			*/
			...
			/**
			* 예외 테스트를 위해 2번째 파일에서 인위적인 예외를 발생시킵니다.
			*/ 
			File isSecondFile = new File(saveDirPath + "isSecondFile.session"); 
			if(!isSecondFile.exists()) { 
				isSecondFile.createNewFile(); 
			}
			else if(isSecondFile.exists() && isSecondFile.isFile()) {
				isSecondFile.delete(); 
				throw new Exception("이것은 인위적인 예외입니다."); 
			}

			/*
			// MAC OS의 Safari, Firefox, Opera 이슈로 파일로 대체
			String isSecondFile = (String)session.getAttribute("isSecondFile");
			if(isSecondFile == null || isSecondFile != "YES") { 
				session.setAttribute("isSecondFile", "YES");
			}
			else { 
				session.setAttribute("isSecondFile", "");
				throw new Exception("이것은 인위적인 예외입니다."); 
			}
			*/ 
