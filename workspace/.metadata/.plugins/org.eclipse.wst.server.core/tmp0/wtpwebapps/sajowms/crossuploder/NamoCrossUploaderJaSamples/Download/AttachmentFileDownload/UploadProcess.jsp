...
<%
	...
	try { 
		...
		if(fileItem != null) { 
			...
			/**
			* DB 등에 업로드 한 파일 정보를 저장합니다. 샘플에서는 session 객체에 파일 정보를 저장하고 있습니다. 
			*/
			session.setAttribute("saveDirPath", fileItem.getLastSavedDirPath());
			session.setAttribute("lastSavedFileName", fileItem.getLastSavedFileName()); 
			session.setAttribute("fileName", fileItem.getFileName()); 
