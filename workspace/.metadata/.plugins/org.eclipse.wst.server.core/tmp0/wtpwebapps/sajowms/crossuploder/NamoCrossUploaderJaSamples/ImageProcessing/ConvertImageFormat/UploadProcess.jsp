		...
		if(fileItem != null) { 
			...
			if(image != null) { 
				...
				*/
				...
				String jpgFilePath = image.convertAs(imageProp, imageSaveDirPath, imageFileName);  
				
				// BMP 포맷으로 이미지 변경
				imageProp.setFormatName("BMP");
				// 저장할 이미지 포맷에 맞는 이름으로 설정
				imageFileName = "Converted_" + fileItem.getFileNameWithoutFileExt() + ".bmp";
				String bmpFilePath = image.convertAs(imageProp, imageSaveDirPath, imageFileName);  
				
				// PNG 포맷으로 이미지 변경
				imageProp.setFormatName("PNG");
				// 저장할 이미지 포맷에 맞는 이름으로 설정
				imageFileName = "Converted_" + fileItem.getFileNameWithoutFileExt() + ".png";
				String pngFilePath = image.convertAs(imageProp, imageSaveDirPath, imageFileName);  
				
				// GIF 포맷으로 이미지 변경
				imageProp.setFormatName("GIF");
				// 저장할 이미지 포맷에 맞는 이름으로 설정
				imageFileName = "Converted_" + fileItem.getFileNameWithoutFileExt() + ".gif";
				String gifFilePath = image.convertAs(imageProp, imageSaveDirPath, imageFileName);  
				...
				// 변경된 이미지 파일 정보 출력
				printConvertedImageInfo(jpgFilePath, bmpFilePath, pngFilePath, gifFilePath, writer); 
			}
			else { 
...
<%!public void printBasicImageInfo(ImageTool image, PrintWriter writer) { 
	...
	public void printConvertedImageInfo(String jpgFilePath, String bmpFilePath, String pngFilePath,
			String gifFilePath, PrintWriter writer) { 

		String html = "<div class='form_area'><table class='form_table'><tbody>"; 
		html += "<tr><td class='form_table_title' colspan=2><b>이미지 포맷 변경 정보</b></td><tr>"; 
		html += "<tr><td>JPG 포맷으로 변경</td><td>" + jpgFilePath + "</td></tr>"; 
		html += "<tr><td>BMP 포맷으로 변경</td><td>" + bmpFilePath + "</td></tr>"; 
		html += "<tr><td>PNG 포맷으로 변경</td><td>" + pngFilePath + "</td></tr>"; 
		html += "<tr><td>GIF 포맷으로 변경</td><td>" + gifFilePath + "</td></tr>"; 
		html += "</tbody></table></div>";
