			...
			if(image != null) { 
				...
				*/
				if(ImageTool.isSupportedImageFormat(inFileItem)) {
					...
					// 다른 유형의 이미지 워터마킹 함수들
					// drawWatermarkImage(String imageFilePath, int x, int y, float transparency); 
					// drawWatermarkImage(File imageFile, int x, int y, float transparency); 
				}
...
<%!public void printBasicImageInfo(ImageTool image, PrintWriter writer) { 
	...
	public void printConvertedImageInfo(String watermarkFilePath, PrintWriter writer) { 
		...
		String html = "<div class='form_area'><table class='form_table'><tbody>"; 
		html += "<tr><td class='form_table_title' colspan=2><b>워터마크 이미지 삽입 정보</b></td><tr>"; 
		html += "<tr><td>워터마깅 된 이미지 파일 경로</td><td>" + watermarkFilePath + "</td></tr>"; 
