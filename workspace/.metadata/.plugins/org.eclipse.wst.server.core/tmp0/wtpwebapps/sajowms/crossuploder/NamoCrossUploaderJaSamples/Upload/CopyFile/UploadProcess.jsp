...
<%
	...
	try { 
		...
		//FileItem fileItem02 = fileUpload.getFileItem("files02");
		//FileItem fileItem03 = fileUpload.getFileItem("files03");
		if(fileItem != null) { 
			...
			/**
			* 현재 파일을 saveDirPath 경로에 fileName 파일명으로 복사합니다. 동일한 파일명이 존재할 경우 다른 이름으로 저장됩니다.
			*/
			...
			// 다른 유형의 복사 함수들
			// copyFile(); 
			// copyFile(String saveDirPath); 
			// copyFile(String saveDirPath, String fileName, boolean overwrite); 
			
			// 업로드 정보 출력
			printFileUploadResult(fileItem, writer); 
			printCopiedFileResult(copiedFilePath01, copiedFilePath02, writer); 
		}
	}
...
<%!
	...
	public void printCopiedFileResult(String copiedFilePath01, String copiedFilePath02, PrintWriter writer) { 
		String html = "<div class='form_area'><table class='form_table'><tbody>"; 
		html += "<tr><td class='form_table_title' colspan=2><b>파일 복사 정보</b></td><tr>"; 
		html += "<tr><td>복사된 파일1</td><td>" + copiedFilePath01 + "</td></tr>"; 
		html += "<tr><td>복사된 파일2</td><td>" + copiedFilePath02 + "</td></tr>"; 
		html += "</tbody></table></div>";
