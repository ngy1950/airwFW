...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//콤보박스 리드온리
		gridList.setReadOnly("gridHeadList", true, ["OWNRKY", "WARESR", "DOCUTY","DIRDVY", "DIRSUP","FILEDN","ORDTYP","PTNG08","C00102", "NAME03B"]);
		gridList.setReadOnly("gridItemList", true, ["WAREKY"]);
		
		uiList.setActive("DL01Link",false);
		uiList.setActive("DL09Link",false);
		uiList.setActive("DL99Link",false);
	...
	function searchList(){
		...
		if(validate.check("searchArea")){
			...
			netUtil.send({
				url : "/outbound/json/displayHeadDL03.data",
				param : param,
				sendType : "list",
	...
	function gridListEventItemGridSearch(gridId, rowNum, itemGridList){
		if(gridId == "gridHeadList"){
			...
			netUtil.send({
				url : "/outbound/json/displayItemDL03.data",
				param : param,
				sendType : "list",
				bindType : "grid",  //bindType grid 고정
...
//		param.put("itemTemp",tempItem);
		netUtil.send({
			url : "/outbound/json/acceptDL03.data",
			param : param,
	...
	function DL01Link(){	
	    ...
	    page.linkPageOpen("DL01", rowData , true);
 	}
...
//		 }
	    ...
	    rowData.put("OWNRKY","ownrky");  
	    rowData.put("WAREKY","wareky");  
	    page.linkPageOpen("DL09", rowData , true);
	...
	function DL99Link(){	
		...
		var rowData = new DataMap();
//		   rowData.put("test","test");  
		   page.linkPageOpen("DL99", rowData , true);
	...
	function successSaveCallBack(json, status){
		if(json && json.data){
			if(json.data != ""){
				...
				uiList.setActive("DL01Link",true);
				uiList.setActive("DL09Link",true);
				uiList.setActive("DL99Link",true);
	...
	function commonBtnClick(btnName){
		if(btnName == "Search"){
			...
			accept();
		}else if(btnName == "AcceptAll"){
			acceptAll();
		}else if(btnName == "DL01Link"){
			DL01Link();
		}else if(btnName == "DL09Link"){
			DL09Link();
		}else if(btnName == "DL99Link"){
			DL99Link();
		}else if(btnName == "Reload"){
			reload();
		}else if(btnName == "Savevariant"){
			sajoUtil.openSaveVariantPop("searchArea", "DL03");
		}else if(btnName == "Getvariant"){
			sajoUtil.openGetVariantPop("searchArea", "DL03");
		}
	}
		...
		<div class="content_serch" id="searchArea">
			<div class="btn_wrap">
				...
				<div class="fl_r">
					...
					<input type="button" CB="AcceptAll ACCEPT_ALL BTN_ACCEPT_ALL" />
					<input type="button" CB="DL01Link DL01_LINK BTN_DL01_LINK" />
					<input type="button" CB="DL09Link DL09_LINK BTN_DL09_LINK" />
					<input type="button" CB="DL99Link DL99_LINK BTN_DL99_LINK" />
			...
			<div class="search_inner" id="searchArea">
				<div class="search_wrap ">
					...
					<dl>  <!--요청수량(BOX)-->  
						...
						<dd> 
							<input type="text" class="input" name="I.QTYORG / SM2.QTDUOM" UIInput="SR"/> 
						</dd> 
					</dl> 
					<dl>  <!--요청수량(낱개)-->  
