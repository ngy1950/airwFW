...
var mode = '';
	$(document).ready(function(){
		...
		$("#SEBELN").keyup(function(e){if(e.keyCode == 13)searchList();});
		$("#SEBELN").focus();
		
// 		//검색값 클릭시 
		$("#SEBELN").click(function(){$(this).select()}) 
// 		//지번 클릭시 
	...
	function gridListEventDataBindEnd(gridId, dataCount){
		if(gridId == "gridList" && dataCount > 0){
			...
			$("#PTNRNM").val(GList[0].get("PTNRKYNM"));
			$("#LOCATG").select();
		}
	}
	...
	function saveData(){
		if(gridList.validationCheck("gridList", "all")){
			...
			var reg = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi;
			docdat = docdat.replace(reg, "");   
	        
			param.put("data", data);		
			param.put("OWNRKY", "<%=ownrky%>");
			param.put("WAREKY", "<%=wareky%>");
			param.put("LOCATG", locatrg);
			param.put("JOBTYP", "110");
			param.put("PRODDT", docdat);
			
			netUtil.send({
				url : "/mobile/MobileInbound/json/saveMGR02.data",
				param : param,
...
<body>
	...
	<div class="mobile_order">
		...
		<div class="m_menutitle"><span>구매입고</span></div>
					...
					<table>
						<tbody>
							<tr>
								...
								<td><input type="text" id="SEBELN" name="SEBELN" class="input input-width"  validate="required(STD_DOCKEY)"/></td>
							</tr>
							<tr>
								<th>거래처명<span class="make"></span></th>
								<td><input type="text" id="PTNRNM" name="PTNRNM" class="input input-width" value="" readonly="readonly"/></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>
									...
									<input type="text" id="WORKSUM2"  name="WORKSUM2" class="input1 input-width" readinly>
								</td>
							</tr>
						</tbody>
					</table>
						...
						<table>
							<tbody id="gridList">
								<tr CGRow="true">
									...
									<td GH="50" GCol="text,QTSIWH" GF="N 13,1"></td>  <!--재고수량-->
