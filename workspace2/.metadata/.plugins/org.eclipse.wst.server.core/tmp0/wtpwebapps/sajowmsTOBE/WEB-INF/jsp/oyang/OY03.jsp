...
<script type="text/javascript">
	$(document).ready(function(){
		...
		gridList.setReadOnly("gridHeadList", true, ["WARESR", "DOCUTY", "ORDTYP", "DIRDVY", "DIRSUP", "C00102"]);
	...
	function commonBtnClick(btnName){
		if(btnName == "Search"){
			...
			sajoUtil.openSaveVariantPop("searchArea", "OY03");
		}else if(btnName == "Getvariant"){
			sajoUtil.openGetVariantPop("searchArea", "OY03");
		}
	}
	...
	function searchList(){
		...
		netUtil.send({
			url : "/OyangOutbound/json/displayOY03.data",
			param : param,
			sendType : "list",
	...
	function saveData() {
		...
		if (gridList.validationCheck("gridHeadList", "select")) {
			...
			for(var i=0; i<head.length; i++){
				var itemMap = head[i].map;

				//OEM 체크
				if(itemMap.REFDKY.trim() != "" && itemMap.WAREKY.trim() == '1H810'){
					commonUtil.msgBox("VALID_SHPOEM"); //OEM발주된 주문은 출고지시해제할 수 없습니다.
					return;
				}
			}

			netUtil.send({
				url : "/OyangOutbound/json/cancelAcceptShpOrder.data",
				param : param,
	...
	function searchHelpEventOpenBefore(searchCode, multyType, $inputObj, rowNum){
		...
		if(searchCode == "SHWAHMA"){
            ...
            param.put("CMCDKY","PGRC02");
            param.put("OWNRKY",$("#OWNRKY").val());
        }else if(searchCode == "SHCMCDV" && $inputObj.name == "SM2.SKUG02"){
            param.put("CMCDKY","SKUG02");
            param.put("OWNRKY",$("#OWNRKY").val());
        }else if(searchCode == "SHCMCDV" && $inputObj.name == "SM2.SKUG03"){
		...
		<div class="content_serch" id="searchArea">
			<div class="btn_wrap">
				...
				<div class="fl_r">
					...
					<input type="button" id="btnSave" CB="Save SAVE BTN_ACCEPT_C" />
				</div>
			</div>
			<div class="search_inner" id="searchArea">
				<div class="search_wrap ">
					...
					<dl>  <!--거래처그룹-->  
						...
						<dd> 
							<input type="text" class="input" name="B2.PTNG01" UIInput="SR"/> 
						</dd> 
					</dl> 
					<dl>  <!--거래처그룹2-->  
						<dt CL="STD_PTNG01G2"></dt> 
						<dd> 
							<input type="text" class="input" name="B2.PTNG02" UIInput="SR"/> 
						</dd> 
					</dl> 
					<dl>  <!--중분류-->  
						<dt CL="STD_SKUG02"></dt> 
						<dd> 
							<input type="text" class="input" name="SM2.SKUG02" UIInput="SR,SHCMCDV"/> 
						</dd> 
					</dl> 
					<dl>  <!--소분류-->  
						<dt CL="STD_SKUG03"></dt> 
						<dd> 
							<input type="text" class="input" name="SM2.SKUG03" UIInput="SR,SHCMCDV"/> 
						</dd> 
					</dl>
				</div> 
				<div class="btn_tab">
							...
							<table class="table_c">
								<tbody id="gridItemList">
									<tr CGRow="true">
			    						...
			    						<td GH="80 IFT_C00103" GCol="select,C00103" GF="S 100">	<!--사유코드-->
