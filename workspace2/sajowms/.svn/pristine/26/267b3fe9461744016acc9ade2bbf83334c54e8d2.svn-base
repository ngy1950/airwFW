...
var portalUserGuBun = '<%=portalUserGb%>'
	...
	$(document).ready(function(){
		...
		gridList.setGrid({
			...
			id : "gridLIst001",
			module : "CL6501",
			command : "CL6501_NOTICE", // BL3302_MT
            ...
            if(json['data']['CL6301_PROJECT'].length>0){
                $.each(json['data']['CL6301_PROJECT'],function(k,v){
                    ...
                    var div = commonUtil.makeElement('div' , '<h2 style="font-size:25px;text-align:center !important;margin-top:40px;font-weight:bold;color:#5ea4ff">' + (dataPostMap.get('TEXT_COL')) + '</h2>' +
						'<h2 style="margin-top:10px;font-size:16px;text-align:center;">' + (dataPostMap.get('PROTOCOL_INFO')||'') + '</h2>' +
						...
						'검사결과  (<span style="color: #f75f31;font-weight:bold;">8</span>) 건</div> ').appendTo(centerElement);
                    div.css({'border-radius':'10px','border':'2px #80beff solid','position':'relative','margin':'15px 10px 0 15px','height':parentContainer});
                });
            } else {
                var arr = [];
                arr.push({'TEXT_COL':'프로젝트 준비중','PROTOCOL_INFO':'준비중'});
                arr.push({'TEXT_COL':'프로젝트 준비중','PROTOCOL_INFO':'준비중'});
                arr.push({'TEXT_COL':'프로젝트 준비중','PROTOCOL_INFO':'준비중'});
                arr.push({'TEXT_COL':'프로젝트 준비중','PROTOCOL_INFO':'준비중'});
                arr.push({'TEXT_COL':'프로젝트 준비중','PROTOCOL_INFO':'준비중'});
		...
		console.log(json , " ====-=-0=-0=-0=-0=-0=-0=-0" , (json['data']['CL6101_POPNOTI']).length)
		console.log(json , " ====-=-0=-0=-0=-0=-0=-0=-0" , (json['data']['CL6101_POPNOTI'])[0])
