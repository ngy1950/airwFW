...
var sLota06;
	...
	$(document).ready(function(){
		...
		// 콤보박스 리드온리
		gridList.setReadOnly("gridItemList", true, ["CONFIRM","LOCATG","ASKU02","SKUG01","SKUG05","LOTA02","PTLT05","PTLT06","TASRSN","LOTA05","LOTA06"]);
	...
	function commonBtnClick(btnName){
		if(btnName == "Search"){
			...
			sajoUtil.openSaveVariantPop("searchArea", "OD10");
		}else if(btnName == "Getvariant"){
			sajoUtil.openGetVariantPop("searchArea", "OD10");
		}
	}// end commonBtnClick()
		...
		if(gridList.validationCheck("gridHeadList", "select")){
			...
			for(var i=0; i<item.length; i++){
				...
				if(sLocasr != "" && sLocasr != itemMap["LOCASR"]){
					commonUtil.msgBox("재고병합은 하나의 재고유형만 가능합니다.");
					return;
				}
				...
				itemMap["LOTA03"] = regExp($("#SEL_LOTA03").val());
				itemMap["LOTA05"] = regExp($("#SEL_LOTA05").val());
				itemMap["LOTA06"] = regExp($("#SEL_LOTA06").val());
				itemMap["LOTA11"] = regExp($("#SEL_LOTA11").val());
				itemMap["LOTA12"] = regExp($("#SEL_LOTA12").val());
				itemMap["LOTA13"] = regExp($("#SEL_LOTA13").val());
			...
			netUtil.send({
				url : "/ConsignOutbound/json/saveOD10.data",
				param : param,
