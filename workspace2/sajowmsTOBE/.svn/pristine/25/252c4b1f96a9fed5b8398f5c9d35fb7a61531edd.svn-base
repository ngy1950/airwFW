...
var SVBELNS = '';
	$(document).ready(function(){
	  ...
	  //OTRQDT 날짜 수정
		...
		setSingleRangeData('S.LOTA06', rangeArr); 
	...
	function searchList(){
		if(validate.check("searchArea")){
			...
			if($('#OWNRKY').val() == '2200' || $('#OWNRKY').val() == '2500'){
				netUtil.send({
					url : "/OutBoundReport/json/displayDL99.data",
					param : param,
					sendType : "list",
...
/* 			if (SKUKEYS != "") {
			...
			netUtil.send({
				url : "/OutBoundReport/json/displayDL99Item.data",
				param : param,
				sendType : "list",
				bindType : "grid",  //bindType grid 고정
	...
	function gridListEventDataBindEnd(gridId, dataCount){
		if(gridId == "gridHeadList" && dataCount == 0){
			...
			gridList.getGridBox("gridHeadList").viewTotal(true);
...
/*  	function gridListEventColValueChange(gridId, rowNum, colName, colValue){
	...
	function saveData(){
		if(gridList.validationCheck("gridItemList", "select")){
			...
			netUtil.send({
				url : "/OutBoundReport/json/saveDL99.data",
				param : param,
	...
	function commonBtnClick(btnName) {
		if (btnName == "Search") {
			...
			sajoUtil.openSaveVariantPop("searchArea", "DL99");
		}else if(btnName == "Getvariant"){
		sajoUtil.openGetVariantPop("searchArea", "DL99");
		}
	}
							...
							<table class="table_c">
								<tbody id="gridHeadList">
									<tr CGRow="true">
			    						...
			    						<td GH="120 STD_QTYPRE2" GCol="text,QTYPRE2" GF="N 80,1">유통선입고수량(낱개)</td>	<!--유통선입고수량(낱개)-->
			    						<td GH="120 STD_BOXPRE2" GCol="text,BOXPRE2" GF="N 80,1">유통선입고수량(BOX)</td>	<!--유통선입고수량(BOX)-->
			    						<td GH="100 STD_BOXWH11" GCol="text,BOXWH1" GF="N 80,1">안성물류(BOX)</td>	<!--안성물류(BOX)-->
			    						<td GH="100 STD_QTYWH11" GCol="text,QTYWH1" GF="N 80,1">안성물류(낱개)</td>	<!--안성물류(낱개)-->
			    						<td GH="100 STD_BOXWH2" GCol="text,BOXWH2" GF="N 80,1">칠서물류(BOX)</td>	<!--칠서물류(BOX)-->
			    						<td GH="100 STD_QTYWH2" GCol="text,QTYWH2" GF="N 80,1">칠서물류(낱개)</td>	<!--칠서물류(낱개)-->
			    						<td GH="100 STD_BOXWH3" GCol="text,BOXWH3" GF="N 80,1">인천물류(BOX)</td>	<!--인천물류(BOX)-->
			    						<td GH="100 STD_QTYWH3" GCol="text,QTYWH3" GF="N 80,1">인천물류(낱개)</td>	<!--인천물류(낱개)-->
			    						<td GH="100 STD_BOXWH4" GCol="text,BOXWH4" GF="N 80,1">양지물류(BOX)</td>	<!--양지물류(BOX)-->
			    						<td GH="100 STD_QTYWH4" GCol="text,QTYWH4" GF="N 80,1">양지물류(낱개)</td>	<!--양지물류(낱개)-->
			    						<td GH="110 STD_BOXWH411" GCol="text,BOXWH4_1" GF="N 80,1">평택세트출고(BOX)</td>	<!--평택세트출고(BOX)-->
			    						<td GH="110 STD_QTYWH41" GCol="text,QTYWH4_1" GF="N 80,1">평택세트출고(낱개)</td>	<!--평택세트출고(낱개)-->
			    						<td GH="110 STD_BOXWH421" GCol="text,BOXWH4_2" GF="N 80,1">미확정수량(BOX)</td>	<!--미확정수량(BOX)-->
			    						<td GH="110 STD_QTYWH421" GCol="text,QTYWH4_2" GF="N 80,1">미확정수량(낱개)</td>	<!--미확정수량(낱개)-->
			    						<td GH="100 STD_BOXWH5" GCol="text,BOXWH5" GF="N 80,1">영천물류(BOX)</td>	<!--영천물류(BOX)-->
			    						<td GH="100 STD_QTYWH5" GCol="text,QTYWH5" GF="N 80,1">영천물류(낱개)</td>	<!--영천물류(낱개)-->
			    						<td GH="100 STD_BOXWH6" GCol="text,BOXWH6" GF="N 80,1">광주물류(BOX)</td>	<!--광주물류(BOX)-->
			    						<td GH="100 STD_QTYWH6" GCol="text,QTYWH6" GF="N 80,1">광주물류(낱개)</td>	<!--광주물류(낱개)-->
			    						<td GH="110 STD_NUM01" GCol="text,NUM01" GF="N 80,1">고성세트출고(낱개)</td>	<!--고성세트출고(낱개)-->
			    						<td GH="110 STD_NUM01BOX" GCol="text,NUM01_BOX" GF="N 80,1">고성세트출고(BOX)</td>	<!--고성세트출고(BOX)-->
			    						<td GH="100 STD_BOXS30" GCol="text,BOXS30" GF="N 80,1">대기재고(BOX)</td>	<!--대기재고(BOX)-->
			    						<td GH="100 STD_QTYS30" GCol="text,QTYS30" GF="N 80,1">대기재고(낱개)</td>	<!--대기재고(낱개)-->
			    						<td GH="160 STD_BOXORD" GCol="text,BOXORD" GF="N 80,1">주문수량+미확정수량(box)</td>	<!--주문수량+미확정수량(box)-->
			    						<td GH="160 STD_QTYORD1" GCol="text,QTYORD" GF="N 80,1">주문수량+미확정수량(낱개)</td>	<!--주문수량+미확정수량(낱개)-->
			    						<td GH="50 STD_BXIQTY" GCol="text,BXIQTY" GF="S 80" style="text-align:right;" >박스입수</td>	<!--박스입수-->
			    						...
			    						<td GH="100 STD_QTSIWHR3" GCol="text,QTSIWH3" GF="N 80,0">부족수량(낱개)</td>	<!--부족수량(낱개)-->
			    						<td GH="110 STD_BOXQTYR4" GCol="text,BOXQTY4" GF="N 80,1">입고예정수량(BOX)</td>	<!--입고예정수량(BOX)-->
			    						<td GH="110 STD_QTSIWHR4" GCol="text,QTSIWH4" GF="N 80,0">입고예정수량(낱개)</td>	<!--입고예정수량(낱개)-->
			    						<td GH="130 STD_BOXQTYR5" GCol="text,BOXQTY5" GF="N 80,1">이고입고예정수량(BOX)</td>	<!--이고입고예정수량(BOX)-->
			    						<td GH="130 STD_QTSIWHR5" GCol="text,QTSIWH5" GF="N 80,0">이고입고예정수량(낱개)</td>	<!--이고입고예정수량(낱개)-->
			    						...
			    						<td GH="120 STD_BOXQTY9" GCol="text,BOXQTY9" GF="N 80,1">생산입고예정량(BOX)</td>	<!--생산입고예정량(BOX)-->
			    						<td GH="120 STD_QTSIWH9" GCol="text,QTSIWH9" GF="N 80,0">생산입고예정량(EA)</td>	<!--생산입고예정량(EA)-->
			    						<td GH="100 STD_SAFQTY" GCol="text,SHPQTY" GF="N 80,0">안전재고수량</td>	<!--안전재고수량-->
								...
								<tbody id="gridItemList">
									<tr CGRow="true">
			    						...
			    						<td GH="200 IFT_WAREKY" GCol="select,WAREKY">
			    							<select class="input" Combo="SajoCommon,WAREKYNM_COMCOMBO_HP"></select>  <!--WMS거점(출고사업장)-->
			    						</td>
			    						...
			    						<td GH="80 IFT_QTYREQ" GCol="input,QTYREQ" GF="N 13,1">납품요청수량</td>	<!--납품요청수량-->
			    						...
			    						<td GH="120 STD_PLTBOX" GCol="text,PLTBOX"  style="text-align:right;">PLT별BOX적재수량</td><!--	PLT별BOX적재수량 -->
			    						...
			    						<td GH="200 STD_IFPGRC04" GCol="select,WARESR">
			    							<select class="input" CommonCombo="PTNG06"><option></option></select>   <!--요청사업장-->
			    						</td>	
			    						...
			    						<td GH="100 IFT_ORDTYP" GCol="text,ORDTYP" GF="S 7">주문/출고형태</td>	<!--주문/출고형태-->
			    						...
			    						<td GH="150 IFT_STATUS" GCol="text,STATUS" GF="S 1">C:신규,M:변경,D:삭제)</td>	<!--C:신규,M:변경,D:삭제)-->
			    						<td GH="200 IFT_TDATE" GCol="text,TDATE" GF="S 14">yyyymmddhhmmss(생성일자)</td>	<!--yyyymmddhhmmss(생성일자)-->
			    						<!-- <td GH="200 IFT_CDATE" GCol="text,CDATE" GF="S 14">yyyymmddhhmmss(처리일자)</td>	yyyymmddhhmmss(처리일자) -->
			    						<!-- <td GH="200 IFT_IFFLG" GCol="text,IFFLG" GF="S 1">Default:N, 처리완료시:Y, 미사용:X</td>	Default:N, 처리완료시:Y, 미사용:X -->
