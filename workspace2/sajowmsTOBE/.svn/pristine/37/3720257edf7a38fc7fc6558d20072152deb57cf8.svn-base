...
public class ExcelSXView extends AbstractView{
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    try {
			...
			Map<String, String> formatNumberMap = (Map)model.get("formatNumberMap");
			Map numberFormatMap = new HashMap();
			Iterator<String> it = formatNumberMap.keySet().iterator();
			while(it.hasNext()){
				String key = it.next();
				String value = formatNumberMap.get(key);
				if(!value.equals("")){
					String[] numberFormat = value.split(",");
					String tmpFormat = "#,##0";
					CellStyle tmpNumberStyle = workbook.createCellStyle();
					tmpNumberStyle.setBorderRight(BorderStyle.THIN);
					tmpNumberStyle.setBorderLeft(BorderStyle.THIN);   
					tmpNumberStyle.setBorderTop(BorderStyle.THIN);   
					tmpNumberStyle.setBorderBottom(BorderStyle.THIN);
					
					if(numberFormat.length > 1){
						
						if(!numberFormat[1].equals("0")){
							tmpFormat += "."; 
							Integer size = Integer.valueOf(numberFormat[1]);
							for(int i=0;i<size;i++){
								tmpFormat += "0";
							}
						}
						
						tmpNumberStyle.setDataFormat(format.getFormat(tmpFormat));
						numberFormatMap.put(key, tmpNumberStyle);
					}				
				}				
			}
			...
			if(model.containsKey(CommonConfig.DATA_EXCEL_COL_KEY_VIEW)){
				...
				createRow(sheet, colList, colMap, dataList, maxRow, formatMap, comboMap, 2+multiRowCount, cellStyle, numberFormatMap);
			}else{
				createRow(sheet, colList, colMap, dataList, maxRow, formatMap, comboMap, 1+multiRowCount, cellStyle, numberFormatMap);
			}
	...
	private void createColumnLabel(Sheet sheet, List<String> labelList, List<JSONArray> multiLabelList, List<String> widthList, CellStyle headCellStyle) throws NumberFormatException, JSONException {
		if(multiLabelList == null){
			...
			for(int i=0;i<labelList.size();i++){
				...
				String widthstr = "";
				if(widthList.get(i).split("\\.").length > 0){
					widthstr = widthList.get(i).split("\\.")[0];
				}else{
					widthstr = widthList.get(i);
				}
				sheet.setColumnWidth(i, Integer.parseInt(widthstr)*(37));
			}
		}else{
	...
	private String getValue(String[] data, Map<String,Integer> colMap, List<String> colList, int j){
		if(colList.get(j).equals("LOTA05")){
			System.out.println("getValue 1 =     " + colMap.get(colList.get(j)));
			System.out.println("getValue 2 =     " + data.length);
		}
	...
	private void createRow(Sheet sheet, List<String> colList, Map<String,Integer> colMap, String[] dataList, int maxRow, Map formatMap, Map comboMap, int startRow, CellStyle cellStyle, Map numberFormatMap) {
		String[] data;
				...
				if(comboMap.containsKey(colList.get(j))){
					...
					if(colMap.containsKey(colList.get(j))){
						if(colList.get(j).equals("LOTA05")){
							...
							}else if(getValue(data, colMap, colList, j).equals(" ")){
					...
					if(formatMap.get(colList.get(j)).equals(CommonConfig.DATA_FORMAT_DATE)){
						...
						if(formatOption[0].equals(CommonConfig.DATA_FORMAT_NUMBER)){
							if(colMap.containsKey(colList.get(j))){
								...
								try{
									String value = getValue(data, colMap, colList, j);
									if(numberFormatMap.containsKey(colList.get(j))){
										CellStyle tmpNumberStyle = (CellStyle)numberFormatMap.get(colList.get(j));
										cell.setCellStyle(tmpNumberStyle);
									}else{
										if(value.indexOf(".") != -1){
