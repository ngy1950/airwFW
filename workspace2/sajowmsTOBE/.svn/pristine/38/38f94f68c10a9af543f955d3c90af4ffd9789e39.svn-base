...
<script type="text/javascript">
	$(document).ready(function(){
		...
		mobileSearchHelp.setSearchHelp({
			...
			search :[
						{"type":"select","label":"STD_AREAKY","name":"AREAKY","combo":"Pda,DEMO_AREACOMBO","codeView":false,"colspan":2},
						[
		...
		if(layerId == "SHZONEKY_LAYER"){
			...
			/* if(isNull(zoneky)){
				...
				mobileCommon.alert({
					...
					confirm : function(){
						mobileCommon.select("","scanArea","ZONEKY");
					}
				});
				return false;
			} */
	...
	function selectSearchHelpAfter(layerId,gridId,data,returnCol,$returnObj){
		if(layerId == "SHZONEKY_LAYER"){
			...
			var searchAreaData = dataBind.paramData("scanArea");
		}else if(layerId == "SHLOCAKY_LAYER"){
			dataBind.dataNameBind(data, "scanArea");
			var searchAreaData = dataBind.paramData("scanArea");
	...
	function comboEventDataBindeBefore(comboAtt,paramName){
		...
		var param = new DataMap();
		if(comboAtt == "Pda,DEMO_AREACOMBO"){
			param.put("WAREKY",wareky);
			return param;
		}
	}
				...
				<table id="scanArea">
					...
					<tbody>
						<tr>
							...
							<td colspan="2">
								<select id="AREAKY" name="AREAKY" Combo="Pda,DEMO_AREACOMBO" UISave="false" ComboCodeView=false></select>
							</td>
						</tr>
						<tr class="searchLine">
