	...
	function saveData(){
		if(gridList.validationCheck("gridItemList", "select")){
			...
			for(var i=0; i<list.length; i++){
				if(list[i].get("LOTA01") == list[i].get("ALOTA01")
				&& list[i].get("LOTA02") == list[i].get("ALOTA02")
				&& list[i].get("LOTA03") == list[i].get("ALOTA03")
				){
					commonUtil.msgBox("SYSTEM_SAVEEMPTY");
					gridList.setRowFocus("gridItemList", list[i].get("GRowNum"), true);
					return;
				}
			}
			...
			netUtil.send({
				url : "/inventory/json/saveSJ04.data",
				param : param,
