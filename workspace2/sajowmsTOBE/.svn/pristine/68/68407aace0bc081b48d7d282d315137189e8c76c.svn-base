...
<script type="text/javascript">
	$(document).ready(function(){
		...
		rangeDataMap2.put(configData.INPUT_RANGE_SINGLE_DATA, "101");
		rangeArr2.push(rangeDataMap2);	

		var rangeDataMap3 = new DataMap();
		rangeDataMap3.put(configData.INPUT_RANGE_OPERATOR,"="); // =  != > < 표시
		rangeDataMap3.put(configData.INPUT_RANGE_SINGLE_DATA, "102");
		rangeArr2.push(rangeDataMap3);	

		var rangeDataMap3 = new DataMap();
		rangeDataMap3.put(configData.INPUT_RANGE_OPERATOR,"="); // =  != > < 표시
		rangeDataMap3.put(configData.INPUT_RANGE_SINGLE_DATA, "110");
		rangeArr2.push(rangeDataMap3);	

		var rangeDataMap3 = new DataMap();
		rangeDataMap3.put(configData.INPUT_RANGE_OPERATOR,"="); // =  != > < 표시
		rangeDataMap3.put(configData.INPUT_RANGE_SINGLE_DATA, "111");
		rangeArr2.push(rangeDataMap3);	
		setSingleRangeData('RH.RCPTTY', rangeArr2);
	...
	function commonBtnClick(btnName){
		if(btnName == "Create"){
			...
			sajoUtil.openSaveVariantPop("searchArea", "GR60");
		}else if(btnName == "Getvariant"){
			sajoUtil.openGetVariantPop("searchArea", "GR60");
		}
	}
		...
		if (gridList.validationCheck("gridHeadList", "data") ) {
			...
			for(var i=0; i<item.length; i++){
				...
				if(itemMap.RSNCOD == ' ' || itemMap.RSNCOD  == ''){
					commonUtil.msgBox("DAERIM_C00102NVL");//* 사유코드는 필수 입력 입니다. *
					return;
				}
			...
			netUtil.send({
				url : "/inbound/json/saveGR60_2.data",
				param : param,
			...
			<div class="search_inner" id="searchArea">
				<div class="search_wrap ">
					...
					<dl>  <!--입고유형-->  
						<dt CL="STD_RCPTTY"></dt> 
						<dd> 
							<input type="text" class="input" name="RH.RCPTTY" UIInput="SR" readonly="readonly"/> 
						</dd> 
					</dl> 
					<dl>  <!--문서상태-->  
						<dt CL="STD_STATDO"></dt> 
					...
					<dl>  <!--유통기한-->  
						<dt CL="STD_LOTA13"></dt> 
						<dd> 
							<input type="text" class="input" name="S.LOTA13" UIFormat="C"  UIInput="B"/> 
						</dd> 
					</dl> 
					<dl>  <!--입고일자-->  
						<dt CL="STD_LOTA12"></dt> 
						<dd> 
							<input type="text" class="input" name="S.LOTA12" UIFormat="C"  UIInput="B"/> 
						</dd> 
					</dl> 
					<dl>  <!--벤더-->  
						<dt CL="STD_LOTA03"></dt> 
						<dd> 
							<input type="text" class="input" name="S.LOTA03" UIInput="SR"/> 
						</dd> 
					</dl> 
					<dl>  <!--포장구분-->  
						<dt CL="STD_LOTA05"></dt> 
						<dd> 
							<input type="text" class="input" name="S.LOTA05" UIInput="SR"/> 
						</dd> 
					</dl> 
					<dl>  <!--재고유형-->  
						<dt CL="STD_LOTA06"></dt> 
						<dd> 
							<input type="text" class="input" name="S.LOTA06" UIInput="SR"/> 
						</dd> 
					</dl>
				</div> 
				<div class="btn_tab">
							...
							<table class="table_c">
								<tbody id="gridHeadList">
									<tr CGRow="true">
			    						...
			    						<td GH="120 STD_ARCPTD" GCol="text,ARCPTD" GF="D 8">입고일자</td>	<!--입고일자-->
							...
							<table class="table_c">
								<tbody id="gridItemList">
									<tr CGRow="true">
			    						...
			    						<td GH="120 STD_RSNCOD" GCol="select,RSNCOD"><!--조정사유코드-->
