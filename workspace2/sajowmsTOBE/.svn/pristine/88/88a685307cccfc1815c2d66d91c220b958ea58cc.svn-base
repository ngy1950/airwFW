...
// 				gridList.setRowReadOnly(gridId, rowNum, true);
	...
	function comboEventDataBindeBefore(comboAtt, $comboObj){
		...
		if(comboAtt == "SajoCommon,DOCUTY_COMCOMBO"){
			param.put("DOCUTY", "131,134,137,138,139");
	...
	function commonBtnClick(btnName){
		if(btnName == "Search"){
			...
			saveData();
	 		...
	 		sajoUtil.openSaveVariantPop("searchArea", "GR46");
 		}else if(btnName == "Getvariant"){
 			sajoUtil.openGetVariantPop("searchArea", "GR46");
		}else if(btnName == "Reprint"){
			print();
		}else if(btnName == "Callback"){
			callback();
		}else if(btnName == "Adjdh"){
			adjdh();
		}
	}
	...
	function adjdh(){
		...
		netUtil.send({
			url : "/SajoInbound/json/saveGR46Adjdh.data",
			param : param,
		...
		<div class="content_serch" id="searchArea">
			<div class="btn_wrap">
				...
				<div class="fl_r">
					...
					<input type="button" CB="Adjdh SAVE 정상재고변경" />
							...
							<table class="table_c">
								<tbody id="gridItemList">
									<tr CGRow="true">
			    						...
			    						<td GH="120 STD_LOCAKY" GCol="input,LOCAKY,SHLOCMA" GF="S 20" validate="required">로케이션</td>	<!--로케이션-->
