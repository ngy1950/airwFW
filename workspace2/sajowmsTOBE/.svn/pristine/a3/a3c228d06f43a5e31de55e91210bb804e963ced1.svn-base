...
<%@ page import="project.common.bean.DataMap,project.common.bean.CommonConfig,java.util.*"%>
...
<%
	...
	String warenm = (String)request.getSession().getAttribute(CommonConfig.SES_USER_WHAREHOUSE_NM_KEY);
	String userid = (String)request.getSession().getAttribute(CommonConfig.SES_USER_ID_KEY);
    String usernm = (String)request.getSession().getAttribute(CommonConfig.SES_USER_NAME_KEY);
...
<script type="text/javascript" src="/common/theme/webdek/js/mobile_theme.js"></script>
<script type="text/javascript" src="/pda/js/mobile.js"></script>
<script type="text/javascript" src="/pda/js/mobileHead.js"></script>
<script type="text/javascript" src="/pda/js/mobileSearchHelp.js"></script>
<script type="text/javascript" src="/pda/js/mobileDatePicker.js"></script>
...
<script type="text/javascript" src="/pda/js/scanInput.js"></script>
<script>
	var moveCount = 0;
	configData.isMobile = true;
	
	var success = new Audio("/pda/sound/success-beep.wav");
	var fail = new Audio("/pda/sound/error-beep.wav");
	...
	window.onpageshow = function (e) {
		if(e.persisted || (window.performance && window.performance.navigation.type == 2)){
			window.parent.location.replace("/pda/index.page?isBack=Y");
		}else{
		...
		mobileCommon.confirm({
			...
			confirm : function(){
				if(menuid == "index"){
					...
					parent.frames["topFrame"].contentWindow.openPage("index", "/pda/info.page");
	...
	function setMenuName(menuId){
		...
		var menuName = "eWMS";
		if(parent.frames["topFrame"].contentWindow.g_menu_list != undefined){
			var menuMap = parent.frames["topFrame"].contentWindow.g_menu_list.get(menuId);
			if(menuMap != undefined){
				menuName = menuMap.get("menuName");
			}
		}
	...
	var gridPageNumList = new DataMap();
	var gridScrollPageTotalList = new DataMap();
</script>
<%
