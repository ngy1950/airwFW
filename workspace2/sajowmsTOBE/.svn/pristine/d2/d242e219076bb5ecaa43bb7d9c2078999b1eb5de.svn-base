...
public class SqlUtil {
	...
	public void setRangeSql(DataMap params){
		if(params.containsKey("RANGE_NAME_LIST")){
			ArrayList<String> rangeNameList =  (ArrayList) params.get("RANGE_NAME_LIST");
			for(int i=0; i <rangeNameList.size(); i++ ){
				setRangeNameSql(params, rangeNameList.get(i));
			}
		}else{
	...
	// setRangeNameSql Range 이름에 따른 처리
	// Cuty JJin 2018.10.10 PJW
	public void setRangeNameSql(DataMap params, String rangeName){
		if(CommonConfig.RNAGE_DATA_PARAM_KEY.equals(rangeName)){
			rangeName = "";
		}
		
		String dataMapName = CommonConfig.RNAGE_DATA_MAP + rangeName;
		String sqlName = CommonConfig.RANGE_SQL_KEY + rangeName;
		String paramName = CommonConfig.RNAGE_DATA_PARAM_KEY + rangeName;
		String ranegePreParam = CommonConfig.RANGE_PRE_PARAM_KEY + rangeName;
		...
		if(params.containsKey(paramName)){
			...
			DataMap rParam = params.getMap(paramName).getMap("map");
			...
			if(sb.length() > 0){
				if(params.containsKey(sqlName)){
					params.put(sqlName, params.get(sqlName)+" AND ( "+sb.toString()+" ) ");
					params.put(ranegePreParam, params.get(ranegePreParam)+" AND ( "+paramSb.toString()+" ) ");
				}else{
					params.put(sqlName, " AND ( "+sb.toString()+" ) ");
					params.put(ranegePreParam, " AND ( "+paramSb.toString()+" ) ");
				}
			}
		
			params.put(ranegePreParam, paramList);	//RANGE_PRE_PARAM
			params.put(dataMapName, rangeData);	//RNAGE_DATA_MAP
			params.remove(paramName);			//RANGE_DATA_PARAM
			
			rParam.setSqlType(false);
		}
	}

//	PGroup 에 관한 스크립트가 존재하지 않아서 주석 처리하고 setRangeNameSql 로 대체 및
//	input.js 스크립트 변경으로 그룹핑 가능하게 수정
//	Ahn JinSeok 2021.01.13
//	public void setRangeSql(DataMap params){
...
//	}
				...
				if(key.indexOf("_"+CommonConfig.RANGE_TYPE_SINGLE) != -1){
					...
					for(int i=0;i<rows.length;i++){		
						...
						if(cols.length < 2){
							...
							rows[i] = rows[i] + "OR";
							cols = rows[i].split(CommonConfig.DATA_COL_SEPARATOR);
						}
	...
	//setRangeSqlStd에서 파싱 가능한 형식의 데이터로 데이터를 변환한다.
	public void convertRangeData(DataMap rangeMap){
		DataMap rOperMap = new DataMap();
		rOperMap.put("=", "E");
		rOperMap.put("!=", "N");
		rOperMap.put("<", "LT");
		rOperMap.put(">", "GT");
		...
		//setRangeSqlStd에서 사용하는 키에 데이터를 담는다.
		rangeMap.put(CommonConfig.RNAGE_DATA_PARAM_KEY, map);
		rangeMap.getMap(CommonConfig.RNAGE_DATA_PARAM_KEY).put("map", dataMap);

		Iterator<String> keys = rangeMap.keySet().iterator();
		String key;
		List<DataMap> rangeList;
		String oper = "";
		while (keys.hasNext()){
			key = keys.next();
			sb = new StringBuffer();
			...
			if(key.indexOf("_"+CommonConfig.RANGE_TYPE_SINGLE) != -1){ //Single
				rangeList = rangeMap.getList(key); 
				//값이 여러개 일 수 있다.
				for(DataMap row : rangeList){

					//부등호 표시  초기화
					//단축 문자열에서 못찾을 경우 
					if(null == operMap.getString(row.getString(CommonConfig.RNAGE_OPERATOR_KEY)) || "".equals(operMap.getString(row.getString(CommonConfig.RNAGE_OPERATOR_KEY)))){
						if(null == rOperMap.getString(row.getString(CommonConfig.RNAGE_OPERATOR_KEY)) || "".equals(rOperMap.getString(row.getString(CommonConfig.RNAGE_OPERATOR_KEY)))){
							oper = "E";
						}else{ //역순에서도 못찾을 경우 
							oper = rOperMap.getString(row.getString(CommonConfig.RNAGE_OPERATOR_KEY));
						}
					}else{ //찾을경우 
						oper = operMap.getString(row.getString(CommonConfig.RNAGE_OPERATOR_KEY));
					}
					
					//row변경시
					if(!"".equals(sb.toString())) sb.append(CommonConfig.DATA_ROW_SEPARATOR);
					
					//col하나의 문자열
					sb.append(oper).append(CommonConfig.DATA_COL_SEPARATOR).append(row.getString(CommonConfig.RNAGE_SINGLE_DATA_KEY)).append(CommonConfig.DATA_COL_SEPARATOR).append(row.getString(CommonConfig.RNAGE_LOGICAL_OPERATOR_KEY));
				}
				...
				for(DataMap row : rangeList){
					...
					//col하나의 문자열
					sb.append(oper).append(CommonConfig.DATA_COL_SEPARATOR).append(row.getString(CommonConfig.RNAGE_FROM_KEY)).append(CommonConfig.DATA_COL_SEPARATOR).append(row.getString(CommonConfig.RNAGE_TO_KEY));
				}
				//값 세팅
				dataMap.put(key,sb.toString());
			}
	...
	//List에 지정한 컬럼을 레인지SQL로 반환한다. 
	public String getRangeSqlFromList(DataMap map, List keyList){
		DataMap data = (DataMap)map.clone();
		//기존 레인지으로부터 List에 지정한 컬럼을 지닌 레인지맵을 생성한다.
		DataMap rangeMap =  this.createRangeMap(data, keyList);
		//생성한 레인지맵을 sql로 변환가능한 String으로 파싱한다.
		this.convertRangeData(rangeMap);
		//레인지를 SQL 문자열로 변환한다.
		this.setRangeSql(rangeMap);
		//생성된 문자열을 반환한다.
		return rangeMap.getMap("map").getString(CommonConfig.RANGE_SQL_KEY);
	}

	
	//List에 지정한 컬럼을 레인지SQL로 반환한다. 특정 값의 알리아스를 변환한다.
	public String getRangeSqlFromListChangeAlias(DataMap map, List keyList, DataMap changeMap){
		DataMap data = (DataMap)map.clone();
		//기존 레인지으로부터 List에 지정한 컬럼을 지닌 레인지맵을 생성한다.
		DataMap rangeMap =  this.createRangeMap(data, keyList);
		
		//rangeMap에서 변환할 알리아스가 있다면 변환한다.
		Iterator<String> keys = changeMap.keySet().iterator();
		String key;
		while (keys.hasNext()){
			key = keys.next();
			this.changeRangeKey(rangeMap, key, changeMap.getString(key));
		}
