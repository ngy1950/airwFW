...
<script type="text/javascript">
	...
	function commonBtnClick(btnName){
		if(btnName == "Search"){
			...
			Print();
		}else if (btnName == "Print2") {	/* 제품/거래처 피킹 */
			Print2();
		}else if (btnName == "Print3") {	/* 품목별 총량집계표  */
			Print3();
		}else if (btnName == "Print4") {	/* 품목별 총량집계표2 */
			Print4();
		}else if(btnName == "Savevariant"){
			sajoUtil.openSaveVariantPop("searchArea", "DR08");
		}else if(btnName == "Getvariant"){
		sajoUtil.openGetVariantPop("searchArea", "DR08");
		}
	}
	...
	function Print(){  /* 거래처/제품 피킹 */
		...
		if(gridList.validationCheck("gridHeadList", "select")){ //체크된 ROW가 있는지 확인
			...
			orderbystr += getMultiRangeDataSQLEzgen('I.WAREKY', 'WAREKY');	
			orderbystr += getMultiRangeDataSQLEzgen('B2.NAME03', 'NAME03');	
			orderbystr += getMultiRangeDataSQLEzgen('I.DOCUTY', 'DOCUTY');	
			orderbystr += getMultiRangeDataSQLEzgen('I.SVBELN', 'SVBELN');	
			orderbystr += getMultiRangeDataSQLEzgen('I.ORDDAT', 'ORDDAT');	
			orderbystr += getMultiRangeDataSQLEzgen('I.OTRQDT', 'OTRQDT');	
			orderbystr += getMultiRangeDataSQLEzgen('I.PTNRTO', 'PTNRTO');
			orderbystr += getMultiRangeDataSQLEzgen('I.PTNROD', 'PTNROD');
			orderbystr += getMultiRangeDataSQLEzgen('I.SKUKEY', 'SKUKEY');
			orderbystr += getMultiRangeDataSQLEzgen('I.DIRSUP', 'DIRSUP');
			orderbystr += getMultiRangeDataSQLEzgen('I.DIRDVY', 'DIRDVY');
			orderbystr += getMultiRangeDataSQLEzgen('C.CARNUM', 'CARNUM');
			orderbystr += getMultiRangeDataSQLEzgen('B2.PTNG08', 'PTNG08');
			orderbystr += getMultiRangeDataSQLEzgen('SM.ASKU05', 'ASKU05');
			orderbystr += getMultiRangeDataSQLEzgen('B.PTNG01', 'PTNG01');
			orderbystr += getMultiRangeDataSQLEzgen('B.PTNG02', 'PTNG02');
			orderbystr += getMultiRangeDataSQLEzgen('B.PTNG03', 'PTNG03');
			orderbystr += getMultiRangeDataSQLEzgen('PK.PICGRP', 'PICGRP');
			orderbystr += getMultiRangeDataSQLEzgen('SM.SKUG03', 'SKUG03');
			orderbystr += getMultiRangeDataSQLEzgen('SW.LOCARV', 'LOCARV');
			...
			WriteEZgenElement("/ezgen/bzptn_picking_sale_list.ezg" , wherestr , " " , langKy, map , width , heigth );

		}
	}
	function Print2(){  /* 제품/거래처 피킹 */
		...
		if(gridList.validationCheck("gridHeadList", "select")){ //체크된 ROW가 있는지 확인
			...
			WriteEZgenElement("/ezgen/product_picking_sale_list.ezg" , wherestr , " " , langKy, map , width , heigth );

		}
	}
	function Print3(){  /* 품목별 총량집계표  */
		...
		if(gridList.validationCheck("gridHeadList", "select")){ //체크된 ROW가 있는지 확인
			...
			WriteEZgenElement("/ezgen/picking_total_sale_list.ezg" , wherestr , " " , langKy, map , width , heigth );

		}
	}
	function Print4(){  /* 품목별 총량집계표2  */
		...
		if(gridList.validationCheck("gridHeadList", "select")){ //체크된 ROW가 있는지 확인
			...
			for(var i =0;i < head.length ; i++){
					...
					var ptng08 = gridList.getColData("gridHeadList", head[i].get("GRowNum"),"PTNG08NM")
					if(ptng08 == "" || ptng08 == null){
						commonUtil.msgBox("마감구분이 없는 데이터가 있습니다.");
						  return;
					   }
			...
			WriteEZgenElement("/ezgen/picking_total_sale_list2.ezg" , wherestr , " " , langKy, map , width , heigth );
		...
		<div class="content_serch" id="searchArea">
			<div class="btn_wrap">
				...
				<div class="fl_r">
					...
					<input type="button" CB="Print4 PRINT_OUT BTN_PRTSKU2" />	<!-- 품목별 총량집계표2 -->
				</div>
			</div>
			<div class="search_inner" id="searchArea">
				<div class="search_wrap ">
					...
					<dl>  <!--피킹출력번호-->  
						<dt CL="STD_PIKSEQ"></dt> 
						<dd> 
							<input type="text" class="input" name="I.TEXT03" UIInput="SR" />
						</dd> 
					</dl> 
					<dl>  <!--출고거점-->  
							...
							<table class="table_c">
								<tbody id="gridHeadList">
									<tr CGRow="true">
										...
										<td GH="200 IFT_PTNRODNM" GCol="text,PTNRODNM" GF="S 20">매출처명</td> <!--매출처명-->
