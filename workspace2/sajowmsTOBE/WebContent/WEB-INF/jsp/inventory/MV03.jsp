...
<script type="text/javascript">
	$(document).ready(function(){
		...
		gridList.setReadOnly("gridItemList", true, ["CONFIRM","LOTA05","LOTA06","SKUG01","ASKU02","SKUG05"]);
	...
	function commonBtnClick(btnName){
		if(btnName == "Search"){
			...
			sajoUtil.openSaveVariantPop("searchArea", "MV03");
		}else if(btnName == "Getvariant"){
			sajoUtil.openGetVariantPop("searchArea", "MV03");
		}
	...
	function saveData() {
		...
		if (gridList.validationCheck("gridHeadList", "data") && gridList.validationCheck("gridItemList", "select") ) {
			...
			param.put("item",item);
			param.put("INDDCL", "V");
			param.put("QTTAOR",$("#qttaorSum").val()); // 작업수량 
			...
			netUtil.send({
				url : "/taskOrder/json/saveMV03.data",
				param : param,
	...
	function saveTaskData(){
		...
		netUtil.send({
			url : "/taskOrder/json/saveTaskMV03.data",
			param : param,
			successFunction : "successSaveCallBack"
		});
	}// end saveTaskData()
		...
		if(gridId == "gridItemList"){
			if(colName == "QTTAOR" || colName == "BOXQTY" || colName == "REMQTY" || colName == "PLTQTY"){ //수량변경시연결된 수량 변경
				...
				if( colName == "QTTAOR" ) {
					...
					boxqty = colValue;
// 				 	remqty = gridList.getColData(gridId, rowNum, "REMQTY");
				 	qttaor = (Number)(bxiqty) * (Number)(boxqty);
				 	pltqty = floatingFloor((Number)(qttaor)/(Number)(pliqty), 2);
				 	//grswgt = qttaor * grswgtcnt;
					...
					gridList.setColValue(gridId, rowNum, "PLTQTY", pltqty);
					gridList.setColValue(gridId, rowNum, "GRSWGT", grswgt);
					gridList.setColValue(gridId, rowNum, "REMQTY", qttaor%(Number)(bxiqty));
	...
	function setChk(type){
    	//부분적용 사유코드  가져오기
		var rsncod = $('#WARECOMBO').val();
		var ptlt06 = $('#LOTA06COMBO').val();
		var ptlt05 = $('#LOTA05COMBO').val();
    	
		if(rsncod == "" && ptlt05 == "" && ptlt06 == ""){
		...
		for(var i=0; i<selectDataList.length; i++){
			if(type == "LOTA06"){
				gridList.setColValue("gridItemList", selectDataList[i].get("GRowNum"), "PTLT06", ptlt06);	// 그리드 재고유형 값 셋팅
			}else if (type == "RSNADJ"){
				gridList.setColValue("gridItemList", selectDataList[i].get("GRowNum"), "RSNCOD", rsncod);	// 그리드 사유코드 값 셋팅
			}else if (type == "LOTA05"){
				gridList.setColValue("gridItemList", selectDataList[i].get("GRowNum"), "PTLT05", ptlt05);	// 그리드 포장구분 값 셋팅
			}
		}
    }
	...
	/* function setAll(){
		...
		var rsncod = $("#WARECOMBO").val();	//인풋값 가져오기
			...
			<div class="content_layout tabs bottom_layout">
				<ul class="tab tab_style02">
					...
					<li style="TOP: 4PX;VERTICAL-ALIGN: middle;PADDING-RIGHT: 10PX"> <!-- 적용 -->
						<input type="button" CB="rsncodChk SAVE BTN_REFLECT" /> 
					</li>
					<!-- 재고유형 반영 -->
					<li style="TOP: 5PX;VERTICAL-ALIGN: middle;">                                                                                                           
						<span CL="STD_PTLT06" style="PADDING-RIGHT: 15PX; VERTICAL-ALIGN: MIDDLE;"></span>                                                                  
					...
					<!-- 포장구분 반영 -->
					<li style="TOP: 5PX;VERTICAL-ALIGN: middle;">                                                                                                           
						<span CL="STD_PTLT05" style="PADDING-RIGHT: 10PX; VERTICAL-ALIGN: MIDDLE;"></span>                                                                  
							...
							<table class="table_c">
								<tbody id="gridItemList">
									<tr CGRow="true">
			    						...
			    						<td GH="110 STD_RSNCOD" GCol="select,RSNCOD">
												<select class="input" Combo="SajoCommon,RSNCOD_COMCOMBO"><option></option></select>
			    						</td>	<!--사유코드-->
			    						<td GH="60 STD_TASRSN" GCol="input,TASRSN" GF="S 127">상세사유</td>	<!--상세사유-->
			    						...
			    						<td GH="80 STD_SYSDAT" GCol="text,SYSDAT" GF="D 14">실물입고일자</td>	<!--실물입고일자-->
			    						<td GH="80 STD_RCPDAT2" GCol="text,RCPDAT" GF="D 14">전산확정일자</td>	<!--전산확정일자-->
									</tr>
								</tbody>
							</table>
						</div>
