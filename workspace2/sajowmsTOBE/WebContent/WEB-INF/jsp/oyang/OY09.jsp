...
<script type="text/javascript">
	...
	function commonBtnClick(btnName){
		if(btnName == "Search"){
			searchList();
		}else if(btnName == "Print"){  //거래처별품목호차
			...
			saveData(); 
		}else if(btnName == "Print2"){  //품목별거래처호차
			if($('#CHKMAK').prop("checked") == true ){
				check = "2";
			}else{
				check = "22";
			...
			saveData(); 
		}else if(btnName == "Print5"){  //총량 피킹 (제품별총량)
			if($('#CHKMAK').prop("checked") == true ){
				check = "5";
			}else{
				check = "55";
			...
			sajoUtil.openSaveVariantPop("searchArea", "OY09");
		}else if(btnName == "Getvariant"){
		sajoUtil.openGetVariantPop("searchArea", "OY09");
		}
	}
	...
	function saveData(){
		if(gridList.validationCheck("gridHeadList", "select")){
			...
			netUtil.send({
				url : "/OyangReport/json/saveOY09.data", //OY08과 같은 쿼리, 서비스
				param : param,
	...
	function returnSAVE(json, status) {
		...
		if(gridList.validationCheck("gridHeadList", "select")){ //체크된 ROW가 있는지 확인
			...
			var height = 880;
				...
				//거래처별품목호차
			if( check == "1" ){
				WriteEZgenElement("/ezgen/bzptn_picking_sale_list2_oy_asku05.ezg" , where , " " , langKy, map , width , height );
			}else if( check == "11" ){
				WriteEZgenElement("/ezgen/bzptn_picking_sale_list2_oy.ezg" , where , " " , langKy, map , width , height );
			
				//품목별거래처호차
			}else if( check == "2" ){
				WriteEZgenElement("/ezgen/product_picking_sale_list2_oy_asku05.ezg" , where , " " , langKy, map , width , height );
		 	}else if( check == "22" ){
		 		WriteEZgenElement("/ezgen/product_picking_sale_list2_oy.ezg" , where , " " , langKy, map , width , height );
		 	
		 		//총량 피킹(오포)
		 	}else if(check == "3" ){
		 		WriteEZgenElement("/ezgen/picking_total_carnum_list_oy_asku05.ezg" , where , " " , langKy, map , width , height );
		 	}else if(check == "33" ){
		 		WriteEZgenElement("/ezgen/picking_total_carnum_list_oy.ezg" , where , " " , langKy, map , width , height );
		 	
		 		//총량 피킹
		 	}else if(check == "4" ){
		 		WriteEZgenElement("/ezgen/picking_total_list_ansan_oy_asku05.ezg" , where , " " , langKy, map , width , height );
		 	}else if(check == "44" ){
		 		WriteEZgenElement("/ezgen/picking_total_list_ansan_oy.ezg" , where , " " , langKy, map , width , height );
		 	
				
				//총량 피킹 (제품별총량)
		 	}else if(check == "5" ){
		 		WriteEZgenElement("/ezgen/picking_total_list_oy_asku05.ezg" , where , " " , langKy, map , width , height );
		 	}else if(check == "55" ){
		 		WriteEZgenElement("/ezgen/picking_total_list_oy.ezg" , where , " " , langKy, map , width , height );
		 	}
		...
		<div class="content_serch" id="searchArea">
			<div class="btn_wrap">
				...
				<div class="fl_r">
					...
					<input type="button" CB="Print PRINT_OUT BTN_CLNT_ITEMS" /> <!-- 거래처별품목호차 -->
					<input type="button" CB="Print2 PRINT_OUT BTN_ITEMS_CLNT" /> <!-- 품목별거래처호차 -->
					<input type="button" CB="Print3 PRINT_OUT BTN_TOLPICKOP" /> <!-- 총량 피킹(오포) -->
					<input type="button" CB="Print4 PRINT_OUT BTN_TOLPICK" /> <!-- 총량 피킹 -->
					<input type="button" CB="Print5 PRINT_OUT BTN_STITOLPICK" /> <!-- 총량 피킹(제품별총량)-->
				</div>
			</div>
